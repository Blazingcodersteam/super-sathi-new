...
export class GSTHelper {
  ...
  static async calculateGST(baseAmount: number, userStateId?: number): Promise<GSTCalculation> {
    ...
    if (gstTypes && gstTypes.length > 0) {
      ...
      if (cgstType) cgstRate = parseFloat(cgstType.tax_percentage) || 9.00;
      if (sgstType) sgstRate = parseFloat(sgstType.tax_percentage) || 9.00;
      if (igstType) igstRate = parseFloat(igstType.tax_percentage) || 18.00;
    }
  ...
  static formatGSTBreakdown(gstCalc: GSTCalculation): any {
    return {
      base_amount: gstCalc.baseAmount.toFixed(2),
      gst_type: gstCalc.gstType,
      ...
      total_gst: gstCalc.totalGstAmount.toFixed(2),
      total_amount: gstCalc.totalAmount.toFixed(2),
      available_gst_types: gstCalc.gstTypes
    };
  }
}
