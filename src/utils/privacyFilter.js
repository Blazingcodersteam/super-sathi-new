...
async function getPrivacySettings(userId) {
    ...
    const [base] = await query(`SELECT u.vivaaha_user_id, up.show_vivaaha_id
     FROM users u
     LEFT JOIN user_profiles up ON up.user_id = u.id
     WHERE u.id = ?`, [userId]);
    const [ps] = await query(`SELECT display_name_privacy, profile_photo_privacy, album_photo_privacy,
...
async function viewerHasLiked(viewerId, profileOwnerId) {
    const [action] = await query(`SELECT id FROM user_interests
     ...
     LIMIT 1`, [viewerId, profileOwnerId]);
    return !!action;
}
// ── Check if viewer is in profile owner's accepted connections ───────────────
async function viewerIsConnected(viewerId, profileOwnerId) {
    const [conn] = await query(`SELECT id FROM connect_now_requests
     WHERE ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?))
       AND status = 'accepted'
     LIMIT 1`, [viewerId, profileOwnerId, profileOwnerId, viewerId]);
    return !!conn;
}
// ── Core single-profile filter ────────────────────────────────────────────────
async function applyPrivacyFilter(matchData, viewerId) {
    if (!matchData || !matchData.id)
        return matchData;
    if (matchData.id === viewerId)
        return matchData; // never filter own profile
    try {
        const ps = await getPrivacySettings(matchData.id);
        // ── Check if viewer is blocked by profile owner (privacy_settings.block_users) ──
        if (ps.block_users) {
            try {
                const blocked = JSON.parse(ps.block_users);
        ...
        const [blockExists] = await query(`SELECT id FROM user_actions
       WHERE ((user_id = ? AND target_user_id = ?) OR (user_id = ? AND target_user_id = ?))
         AND action_type_id = 3
       LIMIT 1`, [matchData.id, viewerId, viewerId, matchData.id]);
        if (blockExists) {
            return null; // completely hide — profile disappears
        }
        ...
        if (ps.hide_profile_from) {
            try {
                const hiddenFrom = JSON.parse(ps.hide_profile_from);
        ...
        const [acct] = await query(`SELECT profile_hidden FROM account_settings WHERE user_id = ?`, [matchData.id]);
        if ((acct === null || acct === void 0 ? void 0 : acct.profile_hidden) === 1) {
            return {
                id: matchData.id,
                is_hidden: true,
                profile_picture: null,
        ...
        const isPremium = async () => {
            if (_isPremium === null)
                _isPremium = await viewerIsPremium(viewerId);
            return _isPremium;
        };
        const hasLiked = async () => {
            if (_hasLiked === null)
                _hasLiked = await viewerHasLiked(viewerId, matchData.id);
            return _hasLiked;
        };
        const isConnected = async () => {
            if (_isConnected === null)
                _isConnected = await viewerIsConnected(viewerId, matchData.id);
            return _isConnected;
        };
        const filtered = Object.assign({}, matchData);
        ...
        if (ppp === 'hide_from_all') {
            ...
            if (Array.isArray(filtered.photos)) {
                filtered.photos = filtered.photos.map((p) => (p.is_primary === 1 || p.is_primary === true)
                    ? Object.assign(Object.assign({}, p), { photo_url: null }) : p // album photos untouched — album_photo_privacy handles them
                );
            }
        }
        else {
            const hideProfilePhoto = (ppp === 'visible_to_premium_only' && !(await isPremium())) ||
            ...
            if (hideProfilePhoto) {
                ...
                if (Array.isArray(filtered.photos)) {
                    filtered.photos = filtered.photos.map((p) => (p.is_primary === 1 || p.is_primary === true) ? Object.assign(Object.assign({}, p), { photo_url: null }) : p);
                }
            }
            else {
        ...
        if (ap === 'hide_from_all') {
            ...
            if (Array.isArray(filtered.photos)) {
                filtered.photos = filtered.photos.map((p) => (p.is_primary === 1 || p.is_primary === true)
                    ? Object.assign({}, p) : Object.assign(Object.assign({}, p), { photo_url: null }));
            }
        }
        ...
        if (pp === 'hide_phone' || pp === 'no_one') {
            ...
            const hidePhone = (pp === 'premium_members' && !(await isPremium())) ||
        ...
        if (ep === 'hide_email') {
            ...
            const hideEmail = (ep === 'all_premium_members' && !(await isPremium())) ||
...
async function applyPrivacyFilterToMatches(matches, viewerId) {
    if (!matches || matches.length === 0)
        return matches;
    const results = await Promise.all(matches
        .filter((m) => m !== null && m !== undefined)
        .map((m) => applyPrivacyFilter(m, viewerId)));
