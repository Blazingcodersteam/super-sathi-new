...
exports.areSubscriptionRestrictionsEnabled = areSubscriptionRestrictionsEnabled;
exports.isCallTypeGloballyEnabled = isCallTypeGloballyEnabled;
exports.invalidateSubscriptionRestrictionsCache = invalidateSubscriptionRestrictionsCache;
exports.hasActiveSubscription = hasActiveSubscription;
exports.isViewerPremium = isViewerPremium;
 ...
 */
async function getGeneralFlags() {
    const now = Date.now();
    if (cachedFlags && now - cachedAt < RESTRICTIONS_CACHE_MS)
        return cachedFlags;
    let flags = {
    ...
    try {
        ...
        flags = {
            subscriptionRestrictions: normalizeFlag(row === null || row === void 0 ? void 0 : row.subscription_restrictions),
            audioRestrictions: normalizeFlag(row === null || row === void 0 ? void 0 : row.audio_restrictions),
            videoRestrictions: normalizeFlag(row === null || row === void 0 ? void 0 : row.video_restrictions),
        };
    }
    catch (error) {
 ...
 */
async function areSubscriptionRestrictionsEnabled() {
 ...
 */
async function isCallTypeGloballyEnabled(callType) {
...
function invalidateSubscriptionRestrictionsCache() {
    cachedFlags = null;
    cachedAt = 0;
}
...
async function hasActiveSubscription(userId) {
    if (!userId)
        return false;
    try {
        const [subscription] = await query(`SELECT id FROM user_subscriptions
        WHERE user_id = ?
          AND is_active = 1
 ...
 */
async function isViewerPremium(userId) {
...
async function getActiveSubscription(userId) {
    if (!userId)
        return null;
    try {
        const [subscription] = await query(`SELECT us.*, sp.plan_name
...
//# sourceMappingURL=subscriptionAccess.js.map