  ...
  try {
    ...
    res.json({ 
      ...
      data: {
        ...
        duration: planData?.duration_months || 0,
        invoice_number: invoiceNumber,
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      try {
        ...
        refundConnection = await new Promise<any>((resolve, reject) => {
          db.getConnection((err: any, conn: any) => {
            if (err) reject(err);
            else resolve(conn);
          });
        ...
        const queryRefundConn = (sql: string, params: any[] = []): Promise<any> => {
          return new Promise((resolve, reject) => {
            refundConnection.query(sql, params, (err: any, results: any) => {
              if (err) reject(err);
              else resolve(results);
            });
          });
        ...
        await new Promise<void>((resolve, reject) => {
          refundConnection.beginTransaction((err: any) => {
            if (err) reject(err);
            else resolve();
          });
        });
        ...
        try {
          ...
          await new Promise<void>((resolve, reject) => {
            refundConnection.commit((err: any) => {
              if (err) reject(err);
              else resolve();
            });
          });
