...
export const testVendorEmail = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const checkEmailConfig = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const settings: any[] = await query(
      "SELECT smtp_host, smtp_port, smtp_username, smtp_from_email, smtp_from_name, smtp_encryption FROM general_settings WHERE id = 1"
    );

    // Check email template
    const template: any[] = await query(
      "SELECT template_key, template_name, status FROM email_templates WHERE template_key = 'vendor_registration_free'"
    );
