...
export const registerVendorFreePlan = async (req: Request, res: Response): Promise<void> => {
  try {
    const {
      category_id,
      full_name,
    ...
    const plan: any[] = await query(
      "SELECT * FROM vendor_subscription_plans WHERE is_active = 1 AND monthly_price = 0 ORDER BY id LIMIT 1"
    );
    ...
    if (languages) {
      if (typeof languages === 'string') {
        ...
        const langArray = languages.split(',').map((lang: string) => lang.trim());
...
export const changeVendorPassword = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = (req as any).user?.id; // From auth middleware
    ...
    const vendor: any[] = await query(
      "SELECT id, email, password FROM vendors WHERE id = ?",
      [vendorId]
    );
    ...
    // Update password
    await query(
      "UPDATE vendors SET password = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
