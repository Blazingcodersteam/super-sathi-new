...
function getFakeScore(user: any): number {
  let score = 0;
  ...
  if (name.length === 1) score += 2;

  const phone = (user.phone || '').replace(/\D/g, '');
  const normalizedPhone = (phone.length === 12 && phone.startsWith('91')) ? phone.slice(2) : phone;
  // All same digits: 9999999999
  if (normalizedPhone.length >= 8 && /^(.)\1+$/.test(normalizedPhone)) score += 3;
  // Sequential digits: 1234567890, 9876543210
  if (/^(0123456789|9876543210|1234567890)$/.test(normalizedPhone)) score += 3;
  // Invalid length
  if (normalizedPhone.length > 0 && (normalizedPhone.length < 7 || normalizedPhone.length > 12)) score += 2;

  return score;
}

function isFakeProfile(user: any): boolean {
...
export async function getAllUsers(req, res) {
  try {
    ...
    const processedUsers = parsedUsers.map(user => ({
      ...user,
      is_fake_profile: isFakeProfile(user)
    }));
...
export async function getUserReports(req, res) {
  try {
    ...
    const [countResult] = await query(
      `SELECT COUNT(DISTINCT ur.id) as total ${baseJoin}`,
      params
    );
...
export async function updateUserStatus(req, res) {
  try {
    ...
    const [existing] = await query(
      "SELECT id, status FROM users WHERE id = ? AND user_type_id = 1",
      [id]
    );
    if (!existing) {
