...
export async function createFormBuilder(req, res) {
  try {
...
export async function getFormBuilderByUser(req, res) {
  try {
      const user_id = req.user.user_id;
...
export async function getFormBuilderByEvent(req, res) {
  try {
    const user_id = req.user.user_id;
    const { event_id,status } = req.body;
    const allStatuses = [
