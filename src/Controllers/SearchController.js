...
exports.searchProfilesComprehensive = searchProfilesComprehensive;
exports.searchProfilesOLD = searchProfilesOLD;
exports.searchProfiles = searchProfiles;
exports.searchNearMe = searchNearMe;
exports.searchByVivahaId = searchByVivahaId;
...
async function searchProfilesComprehensive(req, res) {
    try {
        const userId = req.user.user_id;
        const { min_age, max_age, religion_ids, caste_ids, marital_status_ids, min_income, max_income, mother_tongue_ids, city_ids, education_level_ids, occupation, profile_created_by, min_height, max_height, blood_group_ids, diet_ids, family_type_ids, family_status_ids, family_values_ids, father_occupation_ids, mother_occupation_ids, family_financial_status_ids, gothra_ids, rasi_ids, nakshatra_ids, dosham, state_ids, working_with_ids, currency_ids, income_type, education_area_ids, smoking, drinking, subscription_status, recently_joined, income_range, working_with, profession_area, active_members, match_type, verification_status, photo_settings, country_ids, disability_ids, health_info_ids, profession_ids, has_children, lives_with_family, weight_min, weight_max, manglik_status, eating_habits, community_ids, profile_managed_by_ids, currency_id, number_of_children, page = 1, limit = 10 } = req.query;
        ...
        const [currentUser] = await query("SELECT up.gender_id, up.profile_created_by FROM user_profiles up WHERE up.user_id = ?", [userId]);
        ...
        if ((currentUser === null || currentUser === void 0 ? void 0 : currentUser.profile_created_by) === 'parent') {
        ...
        if (currentUser === null || currentUser === void 0 ? void 0 : currentUser.gender_id) {
    ...
    `;
      ...
      `, [profile.id]);
            return Object.assign(Object.assign({}, profile), { audio_files: audioFiles || [], match_actions: matchActions, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null });
        }));
        // Apply privacy filter to all results
        const filteredProfiles = (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedProfiles, userId)).filter(p => !p.is_blocked);
        res.json({
            success: true,
            data: {
...
async function searchProfilesOLD(req, res) {
    try {
        const userId = req.user.user_id;
        const { min_age, max_age, gender_id, religion_id, caste_id, marital_status_id, min_income, max_income, mother_tongue, city, state, country_id, education_level_id, occupation, profile_created_by, 
        // Physical attributes
        min_height, max_height, blood_group_id, diet_id, disability_id, 
        // Family details
        family_type_id, family_status_id, family_values_id, father_occupation_id, mother_occupation_id, family_financial_status_id, 
        // Astro details
        gothra_id, rasi_id, nakshatra_id, dosham, manglik_status, 
        // Location details
        state_id, city_id, 
        // Career details
        working_with_id, currency_id, income_type, 
        // Education details
        education_area_id, 
        // Lifestyle
        smoking, drinking, 
    ...
    `;
      ...
      `, [userId, profile.id, profile.id, userId]);
            ...
            return Object.assign(Object.assign({}, profile), { family: family || {}, astro: astro || {}, location: location || {}, hobbies: hobbies || [], photos: photos || [], audio_files: audioFiles || [], match_actions: allActions, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null });
        }));
...
async function searchNearMe(req, res) {
    try {
        const userId = req.user.user_id;
        const { radius = 50, page = 1, limit = 10 } = req.query;
        // Get current user's location
        const [currentUserLocation] = await query("SELECT cd.latitude, cd.longitude FROM career_details cd WHERE cd.user_id = ?", [userId]);
      ...
      WHERE u.id != ?
        ...
        ${(currentUser === null || currentUser === void 0 ? void 0 : currentUser.profile_created_by) === 'parent' ? "AND up.profile_created_by = 'parent'" : ''}
        AND u.id NOT IN (
...
async function searchByVivahaId(req, res) {
    try {
        ...
        const [currentUser] = await query("SELECT profile_created_by, gender_id FROM user_profiles WHERE user_id = ?", [userId]);
        const parentFilter = (currentUser === null || currentUser === void 0 ? void 0 : currentUser.profile_created_by) === 'parent'
        ...
        const [profile] = await query(`SELECT u.id, u.email, u.phone, u.email_verified, u.phone_verified, u.vivaaha_user_id, u.created_at,
              up.first_name, up.middle_name, up.last_name, up.gender_id, up.date_of_birth, up.age, up.show_vivaaha_id,
              up.height, up.weight, up.marital_status_id, up.religion_id, up.caste_id,
              up.mother_tongue_id, up.nationality, up.profile_created_by, up.about_me,
              up.about_myself, up.profile_picture, up.horoscope_match, up.lives_with_family,
       ...
       LEFT JOIN currency_master cur ON cd.currency_id = cur.id
       LEFT JOIN profession_master pm ON cd.occupation REGEXP '^[0-9]+$' AND pm.id = CAST(cd.occupation AS UNSIGNED)
       WHERE cd.user_id = ?`, [profile.id]);
        // Get location details
        const [location] = await query(`SELECT ld.*, c.city_name, s.state_name, co.country_name
       FROM location_details ld
       LEFT JOIN cities_master c ON ld.city_id = c.id
       LEFT JOIN states_master s ON ld.state_id = s.id
       LEFT JOIN country_code_master co ON ld.country_id = co.id
       WHERE ld.user_id = ?`, [profile.id]);
        ...
        const [education] = await query(`SELECT ed.*, elm.level_name, eam.area_name
       FROM education_details ed
       LEFT JOIN education_level_master elm ON ed.education_level_id = elm.id
       LEFT JOIN education_area_master eam ON ed.education_area_id = eam.id
       WHERE ed.user_id = ?`, [profile.id]);
        // Get hobbies
        const hobbies = await query(`SELECT hm.* FROM user_hobbies uh
       JOIN hobbies_master hm ON uh.hobby_id = hm.id
       WHERE uh.user_id = ?`, [profile.id]);
        // Get photos with full URLs
        const photos = await query(`SELECT id, photo_url, is_primary, photo_type, upload_date FROM user_photos 
       WHERE user_id = ? ORDER BY is_primary DESC, upload_date DESC`, [profile.id]);
        // Get audio files
        const audioFiles = await query(`SELECT * FROM user_audio_files WHERE user_id = ? ORDER BY created_at DESC`, [profile.id]);
        // Ensure profile_picture has full URL
        if (profile.profile_picture && !profile.profile_picture.startsWith('http')) {
            profile.profile_picture = `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${profile.profile_picture}`;
        }
    ...
    `, [userId, profile.id]);
       ...
       ORDER BY created_at DESC LIMIT 1`, [userId, profile.id, profile.id, userId]);
        ...
        // Record profile view
        await query("INSERT INTO profile_views (viewer_id, viewed_user_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE view_date = CURRENT_TIMESTAMP", [userId, profile.id]);
...
async function getProfileById(req, res) {
    try {
        ...
        const userId = req.user.user_id;
        const [profile] = await query(`SELECT u.id, u.email, u.phone, u.email_verified, u.phone_verified, u.vivaaha_user_id,
              up.first_name, up.middle_name, up.last_name, up.gender_id, up.date_of_birth, up.age, up.show_vivaaha_id,
              CASE WHEN up.show_vivaaha_id = 1 THEN u.vivaaha_user_id ELSE CONCAT(up.first_name, ' ', COALESCE(up.middle_name, ''''), ' ', up.last_name) END as display_name,
              up.height, up.weight, up.marital_status_id, up.religion_id, up.caste_id,
              up.mother_tongue_id, up.nationality, up.profile_created_by, up.about_me,
       ...
       WHERE u.id = ? AND u.id != ?
         -- 21-07-2026 - Profile complete condition
         AND ${(0, profileCompletion_1.profileCompleteCondition)("u", "up")}`, [profileId, userId]);
        if (!profile) {
            return res.status(404).json({
                success: false,
        ...
        const [education] = await query(`SELECT ed.*, elm.level_name FROM education_details ed
       LEFT JOIN education_level_master elm ON ed.education_level_id = elm.id
       WHERE ed.user_id = ?`, [profileId]);
        const [career] = await query(`SELECT cd.*, cm.currency_code,
              CASE WHEN cd.occupation REGEXP '^[0-9]+$' THEN COALESCE(pm.profession_name, cd.occupation) ELSE cd.occupation END as occupation
       FROM career_details cd
       LEFT JOIN currency_master cm ON cd.currency_id = cm.id
       LEFT JOIN profession_master pm ON cd.occupation REGEXP '^[0-9]+$' AND pm.id = CAST(cd.occupation AS UNSIGNED)
       WHERE cd.user_id = ?`, [profileId]);
        const [address] = await query(`SELECT ad.*, cm.country_name FROM address_details ad
       LEFT JOIN country_code_master cm ON ad.country_id = cm.id
       WHERE ad.user_id = ?`, [profileId]);
        // Get family details
        const [family_details] = await query(`SELECT fd.*, po1.occupation_name as father_occupation_name, po2.occupation_name as mother_occupation_name,
              ffs.status_name as financial_status, c.country_name as family_country,
              ft.type_name as family_type, fv.value_name as family_values
       FROM family_details fd
       LEFT JOIN parent_occupation_master po1 ON fd.father_occupation_id = po1.id
       ...
       LEFT JOIN family_type_master ft ON fd.family_type_id = ft.id
       LEFT JOIN family_values_master fv ON fd.family_values_id = fv.id
       WHERE fd.user_id = ?`, [profileId]);
        // Get hobbies
        const hobbies = await query(`SELECT hm.hobby_name, hm.category
       FROM user_hobbies uh
       JOIN hobbies_master hm ON uh.hobby_id = hm.id
       WHERE uh.user_id = ?`, [profileId]);
        // Get audio files
        const audioFiles = await query(`SELECT * FROM user_audio_files WHERE user_id = ? ORDER BY created_at DESC`, [profileId]);
        // Get photos — required for album_photo_privacy (Section 7)
        const photos = await query(`SELECT id, photo_url, is_primary, photo_type, upload_date
       FROM user_photos WHERE user_id = ? ORDER BY is_primary DESC`, [profileId]);
        // Record profile view
        await query("INSERT INTO profile_views (viewer_id, viewed_user_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE view_date = CURRENT_TIMESTAMP", [userId, profileId]);
