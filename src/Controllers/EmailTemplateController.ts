...
export class EmailTemplateController {
  ...
  static async getAllTemplates(req: Request, res: Response) {
    try {
      ...
      const include_deleted = req.query.include_deleted as string || 'true'; // Include deleted by default
  ...
  static async getTemplateByKey(req: Request, res: Response) {
    try {
  ...
  static async createTemplate(req: Request, res: Response) {
    try {
      ...
      const existingTemplate = await query(
        "SELECT id FROM email_templates WHERE template_key = ? AND deleted_at IS NULL",
        [template_key]
      );
  ...
  static async updateTemplate(req: Request, res: Response) {
    try {
      ...
      if (template_key && template_key !== existingTemplate[0].template_key) {
        const duplicateKey = await query(
          "SELECT id FROM email_templates WHERE template_key = ? AND id != ? AND deleted_at IS NULL",
          [template_key, id]
        );
  ...
  static async deleteTemplate(req: Request, res: Response) {
    try {
      ...
      await query(
        "UPDATE email_templates SET deleted_at = CURRENT_TIMESTAMP, status = 'inactive' WHERE id = ?",
        [id]
      );
  ...
  static async restoreTemplate(req: Request, res: Response) {
    try {
      ...
      const existingTemplate = await query(
        "SELECT * FROM email_templates WHERE id = ? AND deleted_at IS NOT NULL",
        [id]
      );
      ...
      await query(
        "UPDATE email_templates SET deleted_at = NULL, status = ? WHERE id = ?",
        [status, id]
      );

      // Get restored template
      const restoredTemplate = await query(
        "SELECT * FROM email_templates WHERE id = ?",
        [id]
      );
  ...
  static async duplicateTemplate(req: Request, res: Response) {
    try {
      ...
      const duplicatedTemplate = await query(
        "SELECT * FROM email_templates WHERE id = ?",
        [result.insertId]
      );
