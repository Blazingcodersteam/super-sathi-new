...
exports.getVendorGSTPreview = exports.getVendorPaymentStatus = exports.handleVendorCCAvenueCancel = exports.handleVendorCCAvenueCallback = exports.verifyVendorRazorpayPayment = exports.initiateVendorPayment = void 0;
...
const VendorCompletionController_1 = require("./VendorCompletionController");
...
const calculateVendorGST = async (baseAmount, billingStateId) => {
...
const initiateVendorPayment = async (req, res) => {
    try {
    ...
    `;
        const tempReg = await query(tempRegQuery, [temp_registration_id]);
        if (tempReg.length === 0) {
            res.status(404).json({
                success: false,
        ...
        const existingPayment = await query("SELECT * FROM vendor_payments WHERE temp_registration_id = ? AND payment_status IN ('pending', 'processing')", [temp_registration_id]);
        ...
        const orderId = payment_method === 'ccavenue'
            ? ccavenue_1.CCAvenue.generateOrderId()
    ...
    `;
        ...
        await query("UPDATE vendor_registration_temp SET status = 'payment_initiated', payment_order_id = ?, payment_method = ?, total_amount = ?, updated_at = CURRENT_TIMESTAMP WHERE temp_id = ?", [orderId, payment_method, gstCalculation.totalAmount, temp_registration_id]);
...
exports.initiateVendorPayment = initiateVendorPayment;
// ============ RAZORPAY ORDER CREATION ============
const createVendorRazorpayOrder = async (registration, registrationData, gstCalculation, paymentId, orderId, res) => {
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                receipt: receipt,
                temp_registration_id: registration.temp_id,
                gst_breakdown: Object.assign(Object.assign({ base_amount: gstCalculation.baseAmount.toFixed(2), gst_type: gstCalculation.gstType }, (gstCalculation.gstType === 'CGST_SGST' ? {
                    cgst: {
                ...
                })), { total_gst: gstCalculation.totalGstAmount.toFixed(2), total_amount: gstCalculation.totalAmount.toFixed(2) })
            }
        });
    }
    catch (error) {
        ...
        await query("UPDATE vendor_payments SET payment_status = 'failed', failure_reason = ? WHERE payment_id = ?", [error.message, paymentId]);
        res.status(500).json({
            success: false,
...
const createVendorCCAvenueOrder = async (registration, registrationData, gstCalculation, paymentId, orderId, res) => {
    try {
...
const verifyVendorRazorpayPayment = async (req, res) => {
    try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, temp_registration_id } = req.body;
        ...
        const paymentRecord = await query("SELECT * FROM vendor_payments WHERE gateway_order_id = ? AND temp_registration_id = ?", [razorpay_order_id, temp_registration_id]);
        ...
        if (expectedSignature !== razorpay_signature) {
            await query("UPDATE vendor_payments SET payment_status = 'failed', failure_reason = 'Invalid signature', updated_at = CURRENT_TIMESTAMP WHERE gateway_order_id = ? AND temp_registration_id = ?", [razorpay_order_id, temp_registration_id]);
            res.status(400).json({
                success: false,
        ...
        try {
            ...
            await query("UPDATE vendor_payments SET payment_status = 'failed', failure_reason = ? WHERE gateway_order_id = ? AND temp_registration_id = ?", [`Razorpay API error: ${razorpayError.message}`, razorpay_order_id, temp_registration_id]);
            res.status(400).json({
                success: false,
        ...
        if (razorpayPayment.status !== 'captured') {
            await query("UPDATE vendor_payments SET payment_status = 'failed', failure_reason = ? WHERE gateway_order_id = ? AND temp_registration_id = ?", [`Payment not captured. Status: ${razorpayPayment.status}`, razorpay_order_id, temp_registration_id]);
            res.status(400).json({
                success: false,
    ...
    `, [
        ...
        await query("UPDATE vendor_registration_temp SET status = 'payment_completed', updated_at = CURRENT_TIMESTAMP WHERE temp_id = ?", [temp_registration_id]);
        // Complete vendor registration
        let registrationResult;
        try {
            registrationResult = await (0, VendorCompletionController_1.completeVendorRegistration)(temp_registration_id, razorpay_payment_id);
            ...
            await query("UPDATE vendor_payments SET gateway_response = ? WHERE gateway_order_id = ? AND temp_registration_id = ?", [JSON.stringify(Object.assign(Object.assign({}, razorpayPayment), { registration_error: registrationError.message })), razorpay_order_id, temp_registration_id]);
            res.status(500).json({
                success: false,
        ...
        try {
            const tempReg = await query("SELECT vrt.*, vsp.plan_name FROM vendor_registration_temp vrt JOIN vendor_subscription_plans vsp ON vrt.plan_id = vsp.id WHERE vrt.temp_id = ?", [temp_registration_id]);
            if (tempReg.length > 0) {
                let registrationData;
                try {
                ...
                await EmailService_1.EmailService.sendVendorPaymentReceiptEmail(registrationData.email, {
...
exports.verifyVendorRazorpayPayment = verifyVendorRazorpayPayment;
// ============ CCAVENUE CALLBACK HANDLERS ============
const handleVendorCCAvenueCallback = async (req, res) => {
    try {
      ...
      `);
        ...
        // Get payment record
        const payment = await query("SELECT * FROM vendor_payments WHERE gateway_order_id = ? AND temp_registration_id = ?", [orderId, tempRegistrationId]);
        if (payment.length === 0) {
      ...
      `, [
            ...
            // Update temp registration status to payment_completed
            await query("UPDATE vendor_registration_temp SET status = 'payment_completed', updated_at = CURRENT_TIMESTAMP WHERE temp_id = ?", [tempRegistrationId]);
            // Complete vendor registration
            await (0, VendorCompletionController_1.completeVendorRegistration)(tempRegistrationId, trackingId);
            // Send payment receipt email
            try {
                const tempReg = await query("SELECT vrt.*, vsp.plan_name FROM vendor_registration_temp vrt JOIN vendor_subscription_plans vsp ON vrt.plan_id = vsp.id WHERE vrt.temp_id = ?", [tempRegistrationId]);
                if (tempReg.length > 0) {
      ...
      `, [
            ...
            try {
                ...
                if (tempReg.length > 0) {
                    const registrationData = JSON.parse(tempReg[0].registration_data);
                    const frontendUrl = process.env.FRONTEND_URL || 'https://vivaaha.net';
                    await EmailService_1.EmailService.sendVendorPaymentFailedEmail(registrationData.email, {
                        vendor_name: registrationData.full_name,
                        plan_name: tempReg[0].plan_name,
...
exports.handleVendorCCAvenueCallback = handleVendorCCAvenueCallback;
const handleVendorCCAvenueCancel = async (req, res) => {
    try {
...
const getVendorPaymentStatus = async (req, res) => {
    try {
      ...
      LIMIT 1
    `;
        const payment = await query(paymentQuery, [temp_registration_id]);
        if (payment.length === 0) {
            res.status(404).json({
                success: false,
                message: "Payment record not found"
...
exports.getVendorPaymentStatus = getVendorPaymentStatus;
// ============ GST PREVIEW FOR VENDOR REGISTRATION ============
const getVendorGSTPreview = async (req, res) => {
    try {
        ...
        // Get plan details
        const plan = await query("SELECT * FROM vendor_subscription_plans WHERE id = ? AND is_active = 1", [plan_id]);
        if (plan.length === 0) {
            res.status(404).json({
                success: false,
        ...
        if (gst_applicable === 'true') {
            gstCalculation = await calculateVendorGST(baseAmount, billing_state_id ? parseInt(billing_state_id) : undefined);
        }
        else {
            gstCalculation = {
                baseAmount,
...
exports.getVendorGSTPreview = getVendorGSTPreview;
