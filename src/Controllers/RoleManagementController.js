...
exports.roleMenuPermissions = exports.menuManagement = exports.roleManagement = void 0;
const utils = require("util");
const db = require("../database");
const query = utils.promisify(db.query).bind(db);
// ============ ROLE MANAGEMENT (user_type_master) ============
exports.roleManagement = {
    ...
    async getAll(req, res) {
        try {
            ...
            const roles = await query(`SELECT utm.*,
         ...
         LIMIT ? OFFSET ?`, [...params, Number(limit), offset]);
            const [{ total }] = await query(`SELECT COUNT(*) as total FROM user_type_master utm ${where}`, params);
            res.json({
                success: true,
                data: roles,
                pagination: {
    ...
    async getById(req, res) {
        try {
            const { id } = req.params;
            const [role] = await query("SELECT * FROM user_type_master WHERE id = ? AND is_active = 1", [id]);
            if (!role) {
                return res.status(404).json({ success: false, message: "Role not found" });
            }
            const menus = await query(`SELECT mm.id, mm.menu_name, mm.menu_slug, mm.menu_icon, mm.parent_id,
         ...
         ORDER BY mm.sort_order ASC`, [id]);
            res.json({ success: true, data: Object.assign(Object.assign({}, role), { menus }) });
        }
        catch (error) {
    ...
    async create(req, res) {
        try {
            ...
            const [existing] = await query("SELECT id FROM user_type_master WHERE type_name = ?", [type_name.toLowerCase()]);
            if (existing) {
                return res.status(400).json({ success: false, message: "Role name already exists" });
            }
            const result = await query("INSERT INTO user_type_master (type_name, description, is_active) VALUES (?, ?, ?)", [type_name.toLowerCase(), description, is_active]);
            res.status(201).json({
                success: true,
    ...
    async update(req, res) {
        try {
            ...
            const [existing] = await query("SELECT id FROM user_type_master WHERE id = ?", [id]);
            if (!existing) {
                return res.status(404).json({ success: false, message: "Role not found" });
            }
            ...
            if (type_name !== undefined) {
                ...
                values.push(type_name.toLowerCase());
            }
            ...
            values.push(id);
            await query(`UPDATE user_type_master SET ${updates.join(", ")} WHERE id = ?`, values);
    ...
    async delete(req, res) {
        try {
            ...
            const [inUse] = await query("SELECT COUNT(*) as cnt FROM users WHERE user_type_id = ?", [id]);
            if (inUse.cnt > 0) {
                return res.status(400).json({
                    success: false,
...
// ============ MENU MANAGEMENT ============
exports.menuManagement = {
    // GET /admin/menus — all menus (tree structure)
    async getAll(req, res) {
        try {
            const menus = await query(`SELECT mm.*, 
         ...
         ORDER BY mm.sort_order ASC, mm.id ASC`);
            ...
            const tree = [];
            menus.forEach((m) => {
                map[m.id] = Object.assign(Object.assign({}, m), { children: [] });
            });
            menus.forEach((m) => {
    ...
    async getById(req, res) {
        try {
            const { id } = req.params;
            const [menu] = await query(`SELECT mm.*, pm.menu_name as parent_name
         FROM menu_master mm
         LEFT JOIN menu_master pm ON mm.parent_id = pm.id
         WHERE mm.id = ? AND mm.is_active = 1`, [id]);
            if (!menu) {
    ...
    async create(req, res) {
        try {
            ...
            const [slugExists] = await query("SELECT id FROM menu_master WHERE menu_slug = ?", [menu_slug]);
            if (slugExists) {
                return res.status(400).json({ success: false, message: "menu_slug already exists" });
            }
            const result = await query(`INSERT INTO menu_master (menu_name, menu_slug, menu_icon, parent_id, sort_order, route_path, is_active)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [menu_name, menu_slug, menu_icon, parent_id || null, sort_order, route_path, is_active]);
            res.status(201).json({
                success: true,
    ...
    async update(req, res) {
        try {
            ...
            if (menu_slug) {
                const [slugExists] = await query("SELECT id FROM menu_master WHERE menu_slug = ? AND id != ?", [menu_slug, id]);
                if (slugExists) {
                    return res.status(400).json({ success: false, message: "menu_slug already exists" });
                }
            }
            ...
            if (menu_slug !== undefined) {
                updates.push("menu_slug = ?");
                values.push(menu_slug);
            }
            if (menu_icon !== undefined) {
                updates.push("menu_icon = ?");
                values.push(menu_icon);
            }
...
exports.roleMenuPermissions = {
    ...
    async getByRole(req, res) {
        try {
            ...
            const [role] = await query("SELECT id, type_name FROM user_type_master WHERE id = ?", [roleId]);
            if (!role) {
                return res.status(404).json({ success: false, message: "Role not found" });
            }
            const permissions = await query(`SELECT mm.id as menu_id, mm.menu_name, mm.menu_slug, mm.menu_icon,
         ...
         INNER JOIN role_menu_permissions rmp
               ON mm.id = rmp.menu_id AND rmp.role_id = ?
         WHERE mm.is_active = 1
         ORDER BY mm.sort_order ASC`, [roleId]);
            // All menus (for the permissions UI — shows what's assigned vs not)
            const allMenus = await query(`SELECT mm.id as menu_id, mm.menu_name, mm.menu_slug, mm.menu_icon,
                mm.parent_id, mm.sort_order, mm.route_path, mm.is_active
         FROM menu_master mm
         WHERE mm.is_active = 1
    ...
    async updateByRole(req, res) {
        try {
            ...
            const [role] = await query("SELECT id FROM user_type_master WHERE id = ?", [roleId]);
            if (!role) {
                return res.status(404).json({ success: false, message: "Role not found" });
            }
            for (const perm of permissions) {
                const { menu_id, can_view = 0, can_create = 0, can_edit = 0, can_delete = 0 } = perm;
                await query(`INSERT INTO role_menu_permissions (role_id, menu_id, can_view, can_create, can_edit, can_delete)
           VALUES (?, ?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE
             ...
             updated_at = CURRENT_TIMESTAMP`, [roleId, menu_id, can_view, can_create, can_edit, can_delete]);
            }
            // Increment permissions_version — forces all staff with this role to re-login
            await query("UPDATE user_type_master SET permissions_version = permissions_version + 1 WHERE id = ?", [roleId]);
    ...
    async deleteOne(req, res) {
        try {
            ...
            await query("DELETE FROM role_menu_permissions WHERE role_id = ? AND menu_id = ?", [roleId, menuId]);
            // Increment permissions_version — forces all staff with this role to re-login
            await query("UPDATE user_type_master SET permissions_version = permissions_version + 1 WHERE id = ?", [roleId]);
    ...
    async getAccessibleMenus(req, res) {
        try {
            ...
            const menus = await query(`SELECT mm.id, mm.menu_name, mm.menu_slug, mm.menu_icon,
         ...
         ORDER BY mm.sort_order ASC`, [role_id]);
            // Build tree
            const map = {};
            const tree = [];
            menus.forEach((m) => { map[m.id] = Object.assign(Object.assign({}, m), { children: [] }); });
            menus.forEach((m) => {
                if (m.parent_id && map[m.parent_id]) {
                    map[m.parent_id].children.push(map[m.id]);
                }
