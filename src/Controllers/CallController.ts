...
import { sendCallNotificationEmail } from './SendMailController';
...
function generateAgoraToken(channelName: string, uid: number): string {
  ...
  return RtcTokenBuilder.buildTokenWithUid(
    ...
    channelName,
    uid,
    RtcRole.PUBLISHER,  // FIX: PUBLISHER so users can send audio/video streams
    privilegeExpiredTs,
    privilegeExpiredTs
  );
}
...
async function autoExpireMissedCalls(): Promise<void> {
  try {
    ...
    if (expiredCalls.length > 0) {
      const expiredIds = expiredCalls.map((c: any) => c.id);
      await query(`
      ...
      for (const call of expiredCalls) {
        ...
        await createCallAlert(
          ...
          'Call Not Answered', 'Your call was not answered'
        );
      }
    }
  } catch (error) {
