...
import { createCallAlert } from './AlertsController';
...
function generateAgoraToken(channelName: string, uid: number): string {
  ...
  return RtcTokenBuilder.buildTokenWithUid(
    ...
    channelName,
    uid,
    RtcRole.PUBLISHER,  // PUBLISHER so users can send audio/video streams
    privilegeExpiredTs,
    privilegeExpiredTs
  );
}

async function validateCallPermission(callerId: number, receiverId: number): Promise<boolean> {
  try {
...
export async function autoExpireMissedCalls(): Promise<number> {
  try {
    // Only fetch calls that are in initiated or ringing status and past the timeout
    const staleCalls: any[] = await query(`
    ...
    for (const call of staleCalls) {
      ...
      if (result && result.affectedRows > 0) {
        ...
        try {
          await createCallAlert(
            call.receiver_id, ALERT_CALL_MISSED, call.caller_id,
            'Missed Call', `You missed a ${call.call_type || 'voice'} call`,
            {}, { call_type: call.call_type, call_id: call.id }
          );
          await createCallAlert(
            call.caller_id, ALERT_CALL_MISSED, call.receiver_id,
            'Call Not Answered', `Your ${call.call_type || 'voice'} call was not answered`,
            {}, { call_type: call.call_type, call_id: call.id }
          );
        } catch (alertErr) {
        ...
        try {
          ...
          if (io) {
            ...
            io.to(`user_${call.caller_id}`).emit('call_ended', { ...expirePayload, ended_by: 'system' });
            io.to(`user_${call.receiver_id}`).emit('call_ended', { ...expirePayload, ended_by: 'system' });
          }
        } catch (socketErr) {
...
export function startCallExpirationWorker(intervalMs: number = CALL_EXPIRATION_INTERVAL_MS): void {
  if (expirationTimer) {
...
export function stopCallExpirationWorker(): void {
  if (expirationTimer) {
...
export async function createCall(req: Request, res: Response) {
  try {
    ...
    const [receiverPrivacy] = await query(
      `SELECT video_call_enabled, voice_call_enabled, availability_time_slot,
    ...
    await createCallAlert(
      receiver_id, ALERT_INCOMING_CALL, userId,
      'Incoming Call', `You have an incoming ${call_type} call`,
      {}, { call_type, call_id: callId }
    );

    try {
      const emailJob = await enqueueEmailOutboxJob({
        jobType: "alert-email",
      ...
      console.error('Failed to queue call notification email:', { callId, message: emailError?.message });
    }
    ...
    try {
      const [receiverUser] = await query(
        'SELECT fcm_token FROM users WHERE id = ?', [receiver_id]
      );
      ...
      if (fcmError?.invalidToken) {
        ...
        console.error('[Call] FCM push error:', fcmError?.message ?? fcmError);
      }
    }
...
export async function acceptCall(req: Request, res: Response) {
  try {
    ...
    // Insert notification into user_alerts
    await createCallAlert(
      call.caller_id, ALERT_CALL_ACCEPTED, userId,
      'Call Accepted', 'Your call has been accepted',
      {}, null
    );
...
export async function declineCall(req: Request, res: Response) {
  try {
    ...
    // Insert notification into user_alerts
    await createCallAlert(
      call.caller_id, ALERT_CALL_DECLINED, userId,
      'Call Declined', 'Your call was declined',
      {}, null
    );
...
export async function endCall(req: Request, res: Response) {
  try {
    ...
    const callDuration = updatedCall?.duration_seconds || 0;

    const otherUserId = userId === call.caller_id ? call.receiver_id : call.caller_id;

    // Insert notification into user_alerts
    await createCallAlert(
      otherUserId, ALERT_CALL_ENDED, userId,
      'Call Ended', 'Call has ended',
      {}, { duration_seconds: callDuration, call_type: call.call_type }
    );
...
export async function getCallHistory(req: Request, res: Response) {
  try {
    ...
    let queryParams: any[] = [userId, userId, userId, userId, userId, userId];
    let countParams: any[] = [userId, userId, userId, userId, userId, userId];
...
export async function getCallHistoryWithUser(req: Request, res: Response) {
  try {
    const userId = (req as any).user.user_id;
    const { user_id } = req.params;
    const otherUserId = parseInt(user_id as string);
...
export async function getIncomingCalls(req: Request, res: Response) {
  try {
    const userId = (req as any).user.user_id;
