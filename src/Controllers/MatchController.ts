...
import { createProfileViewAlert } from "./AlertsController";
...
export async function getMyConnections(req, res) {
  try {
    ...
    const filteredConnections = (await applyPrivacyFilterToMatches(connections, userId)).filter(p => !p.is_blocked);
...
export async function getTodayMatches(req, res) {
  try {
    ...
    const [{ existingCount }] = await query(
      `SELECT COUNT(*) as existingCount FROM matches m
    ...
    if (existingCount > 0) {
      ...
      for (let match of matches) {
        ...
        match.connect_status = connectStatus?.status || null;
      }
      total = existingCount;
    } else {
      ...
      const [myProfile] = await query(
        `SELECT gender_id FROM user_profiles WHERE user_id = ?`,
        [userId]
      );
    ...
    const filteredMatches = (await applyPrivacyFilterToMatches(matches, userId)).filter(p => !p.is_blocked);
...
export async function getMatchProfile(req, res) {
  try {
    ...
    const [profile] = await query(`
      ...
      WHERE u.id = ?
        -- 21-07-2026 - Profile complete condition
        AND ${profileCompleteCondition("u", "up")}`, [userId, matchId]);
    ...
    if (!existing) {
      await createProfileViewAlert(matchId, userId);
    }
    ...
    res.json({
      ...
      profile: {
        ...
        photo_request_status: photoRequestRecord?.status || null
      }
    });
  } catch (error) {
...
export async function compareProfiles(req, res) {
  try {
    ...
    const matchAnalysis = {
      ...
      overall_compatibility: compatibility?.overall_compatibility_score || 0
    };
...
function checkAgeMatch(age, preferences) {
  if (!preferences || !preferences.min_age || !preferences.max_age) return false;
  return age >= preferences.min_age && age <= preferences.max_age;
}

function checkHeightMatch(height, preferences) {
  if (!preferences || !preferences.min_height || !preferences.max_height) return false;
  // Simple height comparison logic
  return true; // Simplified for now
}

function checkReligionMatch(religionId, preferences) {
  if (!preferences || !preferences.religion_ids) return false;
  const religionIds = JSON.parse(preferences.religion_ids || '[]');
  return religionIds.length === 0 || religionIds.includes(religionId);
}

function checkLocationMatch(city, preferences) {
  if (!preferences || !preferences.state) return false;
  return preferences.state.toLowerCase().includes(city?.toLowerCase() || '');
}

function checkEducationMatch(educationId, preferences) {
  if (!preferences || !preferences.education_level_ids) return false;
...
function checkIncomeMatch(income, preferences) {
  if (!preferences || !preferences.min_income) return false;
