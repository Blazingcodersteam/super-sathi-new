...
export async function getDashboard(req, res) {
  try {
    ...
    const [currentUserProfile] = await query(
      `SELECT profile_created_by FROM user_profiles WHERE user_id = ?`, [userId]
    );
    const isParent = currentUserProfile?.profile_created_by === 'parent';
    ...
    const premiumMatchIds = premiumMatches.map((m: any) => m.id);
    ...
    const filteredPremiumMatches = await applyPrivacyFilterToMatches(enrichedPremiumMatches, userId);
    const filteredNewMatches = await applyPrivacyFilterToMatches(enrichedNewMatches, userId);
    const filteredInvitations = await applyPrivacyFilterToMatches(enrichedInvitations, userId);
    ...
    const [recentVisitors] = await query(
      `SELECT COUNT(DISTINCT viewer_id) as count FROM profile_views
       WHERE viewed_user_id = ? AND view_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)`,
      [userId]
    ).catch(() => [{ count: 0 }]);

    const [contactsViewed] = await query(
      `SELECT COUNT(DISTINCT viewed_user_id) as count FROM profile_views WHERE viewer_id = ?`,
      [userId]
    ).catch(() => [{ count: 0 }]);

    const [chatsInitiated] = await query(
      `SELECT COUNT(*) as count FROM chat_conversations WHERE user1_id = ? OR user2_id = ?`,
      [userId, userId]
    ).catch(() => [{ count: 0 }]);
    ...
    res.json({
      ...
      data: {
        ...
        match_actions: {
          sent_interests: sentInterests?.count || 0,
          received_interests: receivedInterests?.count || 0,
          accepted_connections: acceptedConnections?.count || 0
        },
        connect_status: {
          pending_requests: connectRequests?.count || 0
        },
