...
exports.cancelTempRegistration = exports.getTempRegistrationDetails = exports.initiateVendorRegistration = void 0;
...
const initiateVendorRegistration = async (req, res) => {
    try {
        const _a = req.body, { plan_id, payment_method = 'razorpay', // Default to razorpay
        gst_applicable = true, billing_state_id } = _a, registrationData = __rest(_a, ["plan_id", "payment_method", "gst_applicable", "billing_state_id"]);
        ...
        if (registrationData.bank_details && typeof registrationData.bank_details === 'string') {
            try {
                registrationData.bank_details = JSON.parse(registrationData.bank_details);
            }
            catch (_b) {
                registrationData.bank_details = [];
            }
        }
        if (registrationData.services && typeof registrationData.services === 'string') {
            try {
                registrationData.services = JSON.parse(registrationData.services);
            }
            catch (_c) {
        ...
        if (registrationData.documents && typeof registrationData.documents === 'string') {
            try {
                registrationData.documents = JSON.parse(registrationData.documents);
            }
            catch (_d) {
        ...
        // Validate state
        const state = await query("SELECT id, state_name FROM states_master WHERE id = ? AND status = 1", [registrationData.state_id]);
        if (state.length === 0) {
            res.status(400).json({
                success: false,
        ...
        const existingVendor = await query("SELECT id FROM vendors WHERE email = ?", [registrationData.email]);
        ...
        const existingPhone = await query("SELECT id FROM vendors WHERE mobile = ?", [registrationData.phone]);
        ...
        // Validate category
        const category = await query("SELECT id FROM vendor_categories WHERE id = ? AND status = 1", [registrationData.category_id]);
        ...
        const tempId = `VREG_${Date.now()}_${(0, uuid_1.v4)().substring(0, 8)}`;
...
exports.initiateVendorRegistration = initiateVendorRegistration;
// ============ GET TEMP REGISTRATION DETAILS ============
const getTempRegistrationDetails = async (req, res) => {
    try {
    ...
    `;
        const tempReg = await query(tempRegQuery, [temp_id]);
        if (tempReg.length === 0) {
            res.status(404).json({
                success: false,
...
exports.getTempRegistrationDetails = getTempRegistrationDetails;
// ============ CANCEL TEMP REGISTRATION ============
const cancelTempRegistration = async (req, res) => {
    try {
        const { temp_id } = req.params;
        // Get temp registration details
        const tempReg = await query("SELECT * FROM vendor_registration_temp WHERE temp_id = ?", [temp_id]);
        if (tempReg.length === 0) {
            res.status(404).json({
                success: false,
        ...
        await query("UPDATE vendor_registration_temp SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE temp_id = ?", [temp_id]);
        res.json({
            success: true,
...
exports.cancelTempRegistration = cancelTempRegistration;
//# sourceMappingURL=VendorRegistrationController.js.map