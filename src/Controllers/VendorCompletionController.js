...
exports.retryFailedRegistration = exports.getRegistrationDetails = exports.getVendorRegistrations = exports.completeVendorRegistration = void 0;
...
const completeVendorRegistration = async (tempRegistrationId, transactionId) => {
    ...
    `;
        const tempReg = await queryAsync(tempRegQuery, [tempRegistrationId]);
        if (tempReg.length === 0) {
    ...
    `;
        const vendorResult = await queryAsync(insertVendorQuery, [
        ...
        if (uploadedFiles === null || uploadedFiles === void 0 ? void 0 : uploadedFiles.profile_image) {
        ...
        if (profileImagePath) {
            await queryAsync("UPDATE vendors SET profile_picture_url = ? WHERE id = ?", [profileImagePath, vendorId]);
        }
        // Handle document uploads
        if ((uploadedFiles === null || uploadedFiles === void 0 ? void 0 : uploadedFiles.documents) && uploadedFiles.documents.length > 0) {
    ...
    `;
        const subscriptionResult = await queryAsync(insertSubscriptionQuery, [
            vendorId,
            registration.plan_id,
        ...
        await queryAsync("UPDATE vendor_payments SET invoice_url = ?, updated_at = NOW() WHERE temp_registration_id = ?", [invoiceUrl, tempRegistrationId]);
        // Mark temp registration as completed
        await queryAsync("UPDATE vendor_registration_temp SET status = 'completed', vendor_id = ?, subscription_id = ?, completed_at = NOW(), updated_at = NOW() WHERE temp_id = ?", [vendorId, subscriptionId, tempRegistrationId]);
        await new Promise((resolve, reject) => {
        ...
        // Send welcome email
        try {
            await EmailService_1.EmailService.sendVendorRegistrationSuccessEmail(registrationData.email, {
                vendor_name: registrationData.full_name,
...
exports.completeVendorRegistration = completeVendorRegistration;
// ============ GENERATE VENDOR INVOICE PDF ============
const generateVendorInvoice = async (invoiceData) => {
    try {
...
const getVendorRegistrations = async (req, res) => {
    try {
        const query = utils.promisify(db.query).bind(db);
        const { status = 'all', page = 1, limit = 20, search = '', plan_id, date_from, date_to } = req.query;
    ...
    `;
        ...
        const registrations = await query(registrationsQuery, queryParams);
        // Get total count
        const countQuery = `
...
exports.getVendorRegistrations = getVendorRegistrations;
...
const getRegistrationDetails = async (req, res) => {
    try {
      ...
      WHERE vrt.temp_id = ?
    `;
        const details = await query(detailsQuery, [temp_id]);
        if (details.length === 0) {
            res.status(404).json({
                success: false,
...
const retryFailedRegistration = async (req, res) => {
    try {
        const query = utils.promisify(db.query).bind(db);
        const { temp_id } = req.params;
        // Check if registration exists and is in failed state
        const registration = await query("SELECT * FROM vendor_registration_temp WHERE temp_id = ? AND status IN ('payment_failed', 'cancelled')", [temp_id]);
        ...
        await query("UPDATE vendor_registration_temp SET status = 'pending', expires_at = ?, updated_at = NOW() WHERE temp_id = ?", [newExpiryTime, temp_id]);
        // Reset any failed payment records
        await query("UPDATE vendor_payments SET payment_status = 'pending' WHERE temp_registration_id = ? AND payment_status = 'failed'", [temp_id]);
        res.json({
            success: true,
...
exports.retryFailedRegistration = retryFailedRegistration;
//# sourceMappingURL=VendorCompletionController.js.map