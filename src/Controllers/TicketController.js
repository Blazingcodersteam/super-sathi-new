...
exports.createTicketAttendeeGuestLatest = createTicketAttendeeGuestLatest;
...
async function createTicketAttendeeGuestLatest(req, res) {
    try {
        const user_id = req.user.user_id;
        const { event_id, ticket_group_id, attendee_details, guest_details } = req.body;
        const created_at = new Date();
        const ticketGroup = ticket_group_id || `TG-${Date.now()}`;
        // Check for duplicate ticket name
        const [existing] = await query(`SELECT * FROM tickets WHERE event_id = ? AND user_id = ? AND ticket_name = ?`, [event_id, user_id, attendee_details.ticket_name]);
        if (existing) {
    ...
    `;
        ...
        jsonFields.forEach((key) => {
            insertedTicket[key] = (0, Helper_1.tryParseJSON)(insertedTicket[key]);
        });
        ...
        if (insertedTicket.guest_details && typeof insertedTicket.guest_details === "object") {
            ...
            guestFields.forEach((key) => {
                insertedTicket.guest_details[key] = (0, Helper_1.tryParseJSON)(insertedTicket.guest_details[key]);
            });
        }
