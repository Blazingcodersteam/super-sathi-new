...
exports.gstPreview = gstPreview;
exports.publicGstPreview = publicGstPreview;
exports.createOrder = createOrder;
exports.handleCCAvenueCallback = handleCCAvenueCallback;
exports.handleCCAvenueCancel = handleCCAvenueCancel;
...
const gstHelper_1 = require("../utils/gstHelper");
...
async function gstPreview(req, res) {
    try {
        ...
        if (addon_ids && addon_ids.length > 0) {
            ...
            for (const id of ids) {
                const [addon] = await query(`SELECT price FROM subscription_addons_master WHERE id = ? AND is_active = 1`, [id]);
                if (addon)
                    baseAmount += parseFloat(addon.price) || 0;
            }
        }
        const [userLocation] = await query(`SELECT state_id FROM location_details WHERE user_id = ?`, [userId]);
        let gstCalculation;
        try {
            gstCalculation = await gstHelper_1.GSTHelper.calculateGST(baseAmount, userLocation === null || userLocation === void 0 ? void 0 : userLocation.state_id);
        }
        catch (gstError) {
        ...
        res.json({
            ...
            data: {
                ...
                gst_breakdown: gstHelper_1.GSTHelper.formatGSTBreakdown(gstCalculation),
            }
        });
    }
    catch (error) {
...
async function publicGstPreview(req, res) {
    try {
        ...
        let gstCalculation;
        try {
            gstCalculation = await gstHelper_1.GSTHelper.calculateGST(baseAmount, parsedStateId);
        }
        catch (gstError) {
            if (gstError.message && gstError.message.includes('location')) {
                return res.status(400).json({ success: false, message: gstError.message });
...
async function createCCAvenueOrder(userId, planId, plan, gstCalculation, selectedAddons, user, res) {
    const orderId = ccavenue_1.CCAvenue.generateOrderId();
...
async function handleCCAvenueCancel(req, res) {
    try {
        const { encResp } = req.body;
        if (!encResp) {
            return res.status(400).json({
...
//# sourceMappingURL=PaymentControllerDual.js.map