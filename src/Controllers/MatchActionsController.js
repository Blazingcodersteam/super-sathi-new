...
exports.generateUniqueVivahaId = generateUniqueVivahaId;
exports.updateDisplayPreference = updateDisplayPreference;
...
exports.getInterestsSent = getInterestsSent;
exports.getInterestsReceived = getInterestsReceived;
exports.getShortlistedProfiles = getShortlistedProfiles;
exports.getBlockedUsers = getBlockedUsers;
exports.addToShortlist = addToShortlist;
exports.dontShowAgain = dontShowAgain;
exports.blockProfile = blockProfile;
exports.reportProfile = reportProfile;
exports.getReportReasons = getReportReasons;
exports.getRecentlyViewedMembers = getRecentlyViewedMembers;
exports.getWhoViewedMyProfile = getWhoViewedMyProfile;
exports.getIgnoredMembers = getIgnoredMembers;
exports.removeFromIgnored = removeFromIgnored;
exports.connectNow = connectNow;
exports.acceptConnectRequest = acceptConnectRequest;
exports.declineConnectRequest = declineConnectRequest;
exports.cancelConnectRequest = cancelConnectRequest;
exports.getConnectRequests = getConnectRequests;
exports.getInitialMatches = getInitialMatches;
exports.getMatchPlanDetails = getMatchPlanDetails;
exports.getMatchActionSummary = getMatchActionSummary;
exports.bulkMatchActions = bulkMatchActions;
exports.connectWithSelected = connectWithSelected;
...
async function getParentFilter(userId) {
...
async function updateDisplayPreference(req, res) {
    try {
...
async function sendInterest(req, res) {
    try {
        ...
        // Insert interest
        await query("INSERT INTO user_interests (sender_id, receiver_id, message, status) VALUES (?, ?, ?, 'sent') ON DUPLICATE KEY UPDATE message = VALUES(message), status = 'sent', updated_at = CURRENT_TIMESTAMP", [userId, target_user_id, message]);
        const [interestRow] = await query("SELECT id FROM user_interests WHERE sender_id = ? AND receiver_id = ? LIMIT 1", [userId, target_user_id]);
        // Create alert for receiver
        await (0, AlertsController_1.createInterestAlert)(target_user_id, userId, interestRow === null || interestRow === void 0 ? void 0 : interestRow.id);
...
async function acceptInterest(req, res) {
    try {
        ...
        await query("UPDATE user_interests SET status = 'accepted', response_message = ? WHERE id = ? AND receiver_id = ?", [message, interest_id, userId]);
        // Notify the original sender that their interest was accepted
        const [interest] = await query("SELECT sender_id FROM user_interests WHERE id = ?", [interest_id]);
        if (interest)
            await (0, AlertsController_1.createInterestAcceptedAlert)(interest.sender_id, userId, interest_id);
...
async function declineInterest(req, res) {
    try {
        ...
        await query("UPDATE user_interests SET status = 'declined' WHERE id = ? AND receiver_id = ?", [interest_id, userId]);
        // Notify the original sender that their interest was declined
        const [interest] = await query("SELECT sender_id FROM user_interests WHERE id = ?", [interest_id]);
        if (interest)
            await (0, AlertsController_1.createInterestDeclinedAlert)(interest.sender_id, userId, interest_id);
...
async function removeFromShortlist(req, res) {
    try {
        ...
        const { id } = req.params;
        // Remove from shortlist
        await query("DELETE FROM user_actions WHERE user_id = ? AND target_user_id = ? AND action_type_id = 1", [userId, id]);
        // If there is a pending connect request between these two users, decline it
        await query("UPDATE connect_now_requests SET status = 'declined', updated_at = CURRENT_TIMESTAMP WHERE sender_id = ? AND receiver_id = ? AND status = 'pending'", [userId, id]);
...
async function unblockUser(req, res) {
    try {
        ...
        const [isBlocked] = await query("SELECT id FROM user_actions WHERE user_id = ? AND target_user_id = ? AND action_type_id = 3", [userId, targetId]);
        if (!isBlocked) {
            return res.status(400).json({ success: false, message: "User is not blocked" });
        }
        // Remove only the blocker's row (Aâ†’B)
        await query("DELETE FROM user_actions WHERE user_id = ? AND target_user_id = ? AND action_type_id = 3", [userId, targetId]);
...
// Get Interests Sent
async function getInterestsSent(req, res) {
    try {
        const userId = req.user.user_id;
...
async function getInterestsReceived(req, res) {
    try {
        const userId = req.user.user_id;
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
...
async function getShortlistedProfiles(req, res) {
    try {
        const userId = req.user.user_id;
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
    ...
    `, [userId, userId, limit, offset]);
      ...
      `, [userId, profileId, profileId, userId]);
            ...
            if (!profile)
                return null;
            return Object.assign(Object.assign({ shortlisted_date: item.shortlisted_date }, profile), { family: family || {}, astro: astro || {}, location: location || {}, hobbies: hobbies || [], photos: photos || [], match_actions: allActions, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null });
        }));
        const filteredShortlisted = (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedProfiles.filter((p) => p !== null && p !== undefined), userId)).filter(p => !p.is_blocked);
...
async function getBlockedUsers(req, res) {
    ...
    `, [userId, limit, offset]);
      ...
      `, [userId, profileId, profileId, userId]);
            ...
            if (!profile)
                return null;
            return Object.assign(Object.assign({ blocked_date: item.blocked_date }, profile), { family: family || {}, astro: astro || {}, location: location || {}, hobbies: hobbies || [], photos: photos || [], match_actions: allActions, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null });
        }));
        const [{ total }] = await query(`
    ...
    `, [userId]);
        ...
        const validProfiles = enrichedProfiles.filter((p) => p !== null && p !== undefined);
        res.json({
            success: true,
            data: {
...
async function addToShortlist(req, res) {
    ...
    try {
        ...
        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, target_user_id]);
        const [shortlistAction] = await query("SELECT id FROM user_actions WHERE user_id = ? AND target_user_id = ? AND action_type_id = 1 LIMIT 1", [userId, target_user_id]);
        // Section 12 â€” only notify if shortlist_visibility = 'let_others_know' (default)
        // 'do_not_let_others_know' â†’ skip alert entirely
        const [ps] = await query(`SELECT shortlist_visibility FROM privacy_settings WHERE user_id = ?`, [target_user_id]);
        const visibility = (_a = ps === null || ps === void 0 ? void 0 : ps.shortlist_visibility) !== null && _a !== void 0 ? _a : 'let_others_know';
        if (visibility !== 'do_not_let_others_know') {
            await (0, AlertsController_1.createShortlistAlert)(target_user_id, userId, shortlistAction === null || shortlistAction === void 0 ? void 0 : shortlistAction.id);
        }
...
// Don't Show Again
async function dontShowAgain(req, res) {
    try {
        const userId = req.user.user_id;
        const { target_user_id } = req.body;
        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 2) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, target_user_id]);
...
async function blockProfile(req, res) {
    try {
        ...
        const [alreadyBlocked] = await query("SELECT id FROM user_actions WHERE user_id = ? AND target_user_id = ? AND action_type_id = 3", [userId, target_user_id]);
        if (alreadyBlocked) {
        ...
        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 3) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, target_user_id]);
        // Sync privacy_settings.block_users JSON array
        const [existing] = await query("SELECT block_users FROM privacy_settings WHERE user_id = ?", [userId]);
        let blockedUsers = [];
        if (existing && existing.block_users) {
...
async function reportProfile(req, res) {
    try {
        ...
        const validReasons = await query("SELECT id FROM report_reasons_master WHERE id IN (" + reason_ids.map(() => '?').join(',') + ")", reason_ids);
        ...
        await query("INSERT INTO user_reports (reporter_id, reported_user_id, report_reason_ids, report_type, description) VALUES (?, ?, ?, ?, ?)", [userId, target_user_id, JSON.stringify(reason_ids), report_type, description]);
...
async function getReportReasons(req, res) {
    try {
        const reasons = await query("SELECT id, reason_name, parent_id FROM report_reasons_master ORDER BY COALESCE(parent_id, id), CASE WHEN parent_id IS NULL THEN 0 ELSE 1 END, id");
...
async function getRecentlyViewedMembers(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        const [currentUser] = await query(`SELECT gender_id FROM user_profiles WHERE user_id = ?`, [userId]);
        const oppositeGender = (currentUser === null || currentUser === void 0 ? void 0 : currentUser.gender_id) === 1 ? 2 : 1;
      ...
      WHERE
        -- 21-07-2026 - Profile complete condition
        ${(0, profileCompletion_1.profileCompleteCondition)("u", "up")}
    ...
    `, [userId, oppositeGender, userId, parseInt(limit), offset]);
      ...
      `, [userId, profileId, profileId, userId]);
            ...
            if (!profile)
                return null;
            return Object.assign(Object.assign({ view_date: profile.view_date }, profile), { family: family || {}, astro: astro || {}, location: location || {}, hobbies: hobbies || [], photos: photos || [], match_actions: allActions, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null });
        }));
        const [{ total }] = await query(`
      ...
      WHERE pv.viewer_id = ? AND up2.gender_id = ? AND u.status = 1 AND pv.viewed_user_id NOT IN (
        ...
        -- 21-07-2026 - Profile complete condition
        AND ${(0, profileCompletion_1.profileCompleteCondition)("u", "up2")}
    `, [userId, oppositeGender, userId]);
        const filteredRecentlyViewed = (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedProfiles.filter((p) => p !== null && p !== undefined), userId)).filter(p => !p.is_blocked);
        res.json({
            success: true,
            data: {
...
async function getWhoViewedMyProfile(req, res) {
    try {
        const userId = req.user.user_id;
        const { page = 1, limit = 10 } = req.query;
        const offset = (parseInt(page) - 1) * parseInt(limit);
    ...
    `, [userId, oppositeGender, userId, parseInt(limit), offset]);
      ...
      WHERE pv.viewed_user_id = ? AND up2.gender_id = ? AND u.status = 1 AND pv.viewer_id NOT IN (
        ...
        AND ${(0, profileCompletion_1.profileCompleteCondition)("u", "up2")}
    `, [userId, oppositeGender, userId]);
        const filteredViewers = (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedProfiles2.filter((p) => p !== null && p !== undefined), userId)).filter(p => !p.is_blocked);
        res.json({
            success: true,
            data: {
...
async function getIgnoredMembers(req, res) {
    try {
        const userId = req.user.user_id;
        const { page = 1, limit = 10 } = req.query;
        const offset = (parseInt(page) - 1) * parseInt(limit);
    ...
    `, [userId, parseInt(limit), offset]);
      ...
      `, [userId, profileId, profileId, userId]);
            ...
            return {
                ...
                connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null
            };
        }));
        const [{ total }] = await query(`
      SELECT COUNT(*) as total FROM user_actions
...
async function removeFromIgnored(req, res) {
    try {
        const userId = req.user.user_id;
...
// Connect Now
async function connectNow(req, res) {
    var _a;
    const perf = req.connectPerf || null;
    const totalStart = (_a = perf === null || perf === void 0 ? void 0 : perf.startedAt) !== null && _a !== void 0 ? _a : connectPerfNow();
    try {
        const userId = req.user.user_id;
        const { target_user_id, message } = req.body;
        if (perf) {
        ...
        const connectionResult = await query("INSERT INTO connect_now_requests (sender_id, receiver_id, message, status) VALUES (?, ?, ?, 'pending') ON DUPLICATE KEY UPDATE message = VALUES(message), status = 'pending', updated_at = CURRENT_TIMESTAMP", [userId, target_user_id, message]);
        const [connectionRow] = await query("SELECT id FROM connect_now_requests WHERE sender_id = ? AND receiver_id = ? LIMIT 1", [userId, target_user_id]);
        const connectionRequestId = (connectionRow === null || connectionRow === void 0 ? void 0 : connectionRow.id) || (connectionResult === null || connectionResult === void 0 ? void 0 : connectionResult.insertId);
        if (perf && connectionRequestId)
            perf.connectionId = connectionRequestId;
        logConnectPerf(perf, "create-connection-db-operation", createConnectionStart);
        // Create alert for receiver
        const notificationStart = perf ? connectPerfNow() : 0;
        await (0, AlertsController_1.createConnectNowAlert)(target_user_id, userId, perf, connectionRequestId);
        ...
        if (perf)
            console.log(`[CONNECT-PERF] ${connectPerfMeta(perf)} TOTAL API TIME: ${connectPerfElapsed(totalStart)}ms (error)`);
        res.status(500).json({ success: false, message: "Server error" });
    }
}
...
async function acceptConnectRequest(req, res) {
    try {
        ...
        const result = await query("UPDATE connect_now_requests SET status = 'accepted', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND receiver_id = ? AND status = 'pending'", [request_id, userId]);
        ...
        const [req2] = await query("SELECT sender_id FROM connect_now_requests WHERE id = ?", [request_id]);
        if (req2)
            await (0, AlertsController_1.createConnectAcceptedAlert)(req2.sender_id, userId, request_id);
...
async function declineConnectRequest(req, res) {
    try {
        ...
        const result = await query("UPDATE connect_now_requests SET status = 'declined', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND receiver_id = ? AND status = 'pending'", [request_id, userId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({
                success: false,
                message: "Request not found, already processed, or you are not the receiver"
        ...
        const [req2] = await query("SELECT sender_id FROM connect_now_requests WHERE id = ?", [request_id]);
        if (req2)
            await (0, AlertsController_1.createConnectDeclinedAlert)(req2.sender_id, userId, request_id);
...
async function cancelConnectRequest(req, res) {
    try {
        ...
        let result;
        if (request_id) {
            result = await query("UPDATE connect_now_requests SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND sender_id = ? AND status IN ('pending', 'accepted')", [request_id, userId]);
        }
        else if (target_user_id) {
            // Allow cancelling both pending and accepted connections (either direction)
            result = await query("UPDATE connect_now_requests SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) AND status IN ('pending', 'accepted')", [userId, target_user_id, target_user_id, userId]);
        }
        else {
...
async function getConnectRequests(req, res) {
    try {
        const userId = req.user.user_id;
    ...
    `, [userId, userId, userId]);
        res.json({ success: true, data: (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(requests, userId)).filter(p => !p.is_blocked) });
    }
    catch (error) {
        res.status(500).json({ success: false, message: "Server error" });
    }
...
async function getInitialMatches(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        const hasSubscription = await (0, subscriptionAccess_1.isViewerPremium)(userId);
    ...
    `, queryParams);
      ...
      `, [userId, profileId]);
            ...
            return Object.assign(Object.assign({}, profile), { family: family || {}, astro: astro || {}, location: location || {}, hobbies: hobbies || [], photos: photos || [], match_percentage: matchPercentage, connect_status: (connectStatus === null || connectStatus === void 0 ? void 0 : connectStatus.status) || null, match_actions: {
                    shortlist: {
                        is_shortlisted: !!shortlistStatus,
                        date: (shortlistStatus === null || shortlistStatus === void 0 ? void 0 : shortlistStatus.created_at) || null
                    },
                    interest: {
                        status: (interestStatus === null || interestStatus === void 0 ? void 0 : interestStatus.status) || null,
                        date: (interestStatus === null || interestStatus === void 0 ? void 0 : interestStatus.created_at) || null,
                        message: (interestStatus === null || interestStatus === void 0 ? void 0 : interestStatus.message) || null
                    },
                    block: {
                        is_blocked: !!blockedStatus,
                        date: (blockedStatus === null || blockedStatus === void 0 ? void 0 : blockedStatus.created_at) || null
                    },
                    ignore: {
                        is_ignored: !!ignoredStatus,
                        date: (ignoredStatus === null || ignoredStatus === void 0 ? void 0 : ignoredStatus.created_at) || null
                    }
                } });
        }));
        // â”€â”€ Apply privacy filter to all matches (DOB, income, photo, phone, email)
        const filteredMatches = await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedMatches.filter(Boolean), userId);
        res.json({
            success: true,
            data: {
...
function calculateMatchPercentage(profile, family, astro, location) {
    ...
    let totalCriteria = 0;
    // Religion match (20%)
    if (profile === null || profile === void 0 ? void 0 : profile.religion_id) {
        totalCriteria += 20;
        // This would need user's preferences to compare
        score += 15; // Default partial match
    }
    // Caste match (15%)
    if (profile === null || profile === void 0 ? void 0 : profile.caste_id) {
        totalCriteria += 15;
        score += 10; // Default partial match
    }
    // Location match (15%)
    if (location === null || location === void 0 ? void 0 : location.city_id) {
        totalCriteria += 15;
        score += 12; // Default partial match
    }
    // Education match (15%)
    if (profile === null || profile === void 0 ? void 0 : profile.education_level) {
        totalCriteria += 15;
        score += 12; // Default partial match
    }
    ...
    // Income match (10%)
    if (profile === null || profile === void 0 ? void 0 : profile.annual_income) {
        totalCriteria += 10;
        score += 7; // Default partial match
    }
    // Family values (10%)
    if (family === null || family === void 0 ? void 0 : family.family_type) {
        totalCriteria += 10;
        score += 8; // Default partial match
    }
    // Horoscope match (5%)
    if (astro === null || astro === void 0 ? void 0 : astro.rasi) {
        totalCriteria += 5;
...
async function getMatchPlanDetails(req, res) {
    try {
        const userId = req.user.user_id;
    ...
    `, [userId, userId]);
        const planDetails = {
            ...
            usage: {
                matches_viewed_today: (todayUsage === null || todayUsage === void 0 ? void 0 : todayUsage.matches_viewed_today) || 0,
                matches_viewed_this_month: (monthlyUsage === null || monthlyUsage === void 0 ? void 0 : monthlyUsage.matches_viewed_this_month) || 0,
                interests_sent_today: (interestsSentToday === null || interestsSentToday === void 0 ? void 0 : interestsSentToday.interests_sent_today) || 0,
                available_matches: (availableMatches === null || availableMatches === void 0 ? void 0 : availableMatches.available_matches) || 0
            },
            limits: {
                daily_matches_remaining: Math.max(0, ((subscription === null || subscription === void 0 ? void 0 : subscription.daily_matches_limit) || 5) - ((todayUsage === null || todayUsage === void 0 ? void 0 : todayUsage.matches_viewed_today) || 0)),
                monthly_matches_remaining: Math.max(0, ((subscription === null || subscription === void 0 ? void 0 : subscription.monthly_matches_limit) || 50) - ((monthlyUsage === null || monthlyUsage === void 0 ? void 0 : monthlyUsage.matches_viewed_this_month) || 0)),
                can_send_more_interests: (subscription === null || subscription === void 0 ? void 0 : subscription.can_send_unlimited_interests) || ((interestsSentToday === null || interestsSentToday === void 0 ? void 0 : interestsSentToday.interests_sent_today) || 0) < 3
            }
        };
...
async function getMatchActionSummary(req, res) {
    try {
        const userId = req.user.user_id;
    ...
    `, [userId]);
        const summary = {
            interests: {
                sent: {
                    total: (interestsSent === null || interestsSent === void 0 ? void 0 : interestsSent.total_sent) || 0,
                    accepted: (interestsSent === null || interestsSent === void 0 ? void 0 : interestsSent.accepted) || 0,
                    declined: (interestsSent === null || interestsSent === void 0 ? void 0 : interestsSent.declined) || 0,
                    pending: (interestsSent === null || interestsSent === void 0 ? void 0 : interestsSent.pending) || 0
                },
                received: {
                    total: (interestsReceived === null || interestsReceived === void 0 ? void 0 : interestsReceived.total_received) || 0,
                    accepted: (interestsReceived === null || interestsReceived === void 0 ? void 0 : interestsReceived.accepted) || 0,
                    declined: (interestsReceived === null || interestsReceived === void 0 ? void 0 : interestsReceived.declined) || 0,
                    pending: (interestsReceived === null || interestsReceived === void 0 ? void 0 : interestsReceived.pending) || 0
                }
            },
            connects: {
                sent: {
                    total: (connectsSent === null || connectsSent === void 0 ? void 0 : connectsSent.total_sent) || 0,
                    accepted: (connectsSent === null || connectsSent === void 0 ? void 0 : connectsSent.accepted) || 0,
                    declined: (connectsSent === null || connectsSent === void 0 ? void 0 : connectsSent.declined) || 0,
                    pending: (connectsSent === null || connectsSent === void 0 ? void 0 : connectsSent.pending) || 0
                },
                received: {
                    total: (connectsReceived === null || connectsReceived === void 0 ? void 0 : connectsReceived.total_received) || 0,
                    accepted: (connectsReceived === null || connectsReceived === void 0 ? void 0 : connectsReceived.accepted) || 0,
                    declined: (connectsReceived === null || connectsReceived === void 0 ? void 0 : connectsReceived.declined) || 0,
                    pending: (connectsReceived === null || connectsReceived === void 0 ? void 0 : connectsReceived.pending) || 0
                }
            },
            actions: {
                shortlisted: (shortlistCount === null || shortlistCount === void 0 ? void 0 : shortlistCount.total_shortlisted) || 0,
                blocked: (blockedCount === null || blockedCount === void 0 ? void 0 : blockedCount.total_blocked) || 0,
                ignored: (ignoredCount === null || ignoredCount === void 0 ? void 0 : ignoredCount.total_ignored) || 0
            },
            profile_activity: {
                total_views: (profileViews === null || profileViews === void 0 ? void 0 : profileViews.total_views) || 0,
                unique_profiles_viewed: (profileViews === null || profileViews === void 0 ? void 0 : profileViews.unique_profiles_viewed) || 0
            }
        };
...
// Bulk Match Actions
async function bulkMatchActions(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        for (const targetUserId of target_user_ids) {
            try {
                switch (action_type) {
                    case 'shortlist':
                        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, targetUserId]);
                        break;
                    case 'interest':
                        await query("INSERT INTO user_interests (sender_id, receiver_id, message, status) VALUES (?, ?, ?, 'sent') ON DUPLICATE KEY UPDATE message = VALUES(message), updated_at = CURRENT_TIMESTAMP", [userId, targetUserId, message || "Hi! I'm interested in your profile."]);
                        break;
                    case 'connect':
                        await query("INSERT INTO connect_now_requests (sender_id, receiver_id, message, status) VALUES (?, ?, ?, 'pending') ON DUPLICATE KEY UPDATE message = VALUES(message), status = 'pending', updated_at = CURRENT_TIMESTAMP", [userId, targetUserId, message || "Hi! I would like to connect with you."]);
                        break;
                    case 'ignore':
                        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 2) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, targetUserId]);
                        break;
                    case 'block':
                        await query("INSERT INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 3) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP", [userId, targetUserId]);
                        break;
                    default:
...
async function connectWithSelected(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        const ignoredBlockedUsers = await query("SELECT target_user_id FROM user_actions WHERE user_id = ? AND action_type_id IN (2, 3)", [userId]);
        ...
        for (const targetUserId of selected_user_ids) {
            try {
                ...
                await query("INSERT INTO connect_now_requests (sender_id, receiver_id, message, status) VALUES (?, ?, ?, 'pending') ON DUPLICATE KEY UPDATE message = VALUES(message), status = 'pending', updated_at = CURRENT_TIMESTAMP", [userId, targetUserId, message]);
                const [connectionRow] = await query("SELECT id FROM connect_now_requests WHERE sender_id = ? AND receiver_id = ? LIMIT 1", [userId, targetUserId]);
                // Create alert for each receiver
                await (0, AlertsController_1.createConnectNowAlert)(targetUserId, userId, null, connectionRow === null || connectionRow === void 0 ? void 0 : connectionRow.id);
...
async function getMyConnections(req, res) {
    try {
      ...
      ORDER BY cnr.created_at DESC
      LIMIT ? OFFSET ?
    `, [userId, userId, userId, userId, userId, userId, userId, userId, parseInt(limit), offset]);
    ...
    `, [userId, userId, userId, userId, userId]);
        const filteredConnections = (await (0, privacyFilter_1.applyPrivacyFilterToMatches)(enrichedConnections.filter((c) => c !== null && c !== undefined), userId)).filter(p => !p.is_blocked);
        res.json({
            success: true,
            data: {
...
async function getConnectionStatus(req, res) {
    try {
        ...
        const [request] = await query(`SELECT cnr.id, cnr.sender_id, cnr.receiver_id, cnr.status,
       ...
       LIMIT 1`, [target_user_id, target_user_id, userId, target_user_id, target_user_id, userId]);
        if (!request) {
            return res.json({
                success: true,
                data: {
...
//# sourceMappingURL=MatchActionsController.js.map