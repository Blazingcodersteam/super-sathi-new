...
export const createConsultation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const vendor: any[] = await query(
      "SELECT id, full_name, email FROM vendors WHERE id = ?",
      [vendorId]
    );
...
export const getMyConsultations = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = (req as any).user.id;
    const { page = 1, limit = 10, status, payment_status, search } = req.query;
    
    let whereClause = "WHERE vendor_id = ?";
    let queryParams: any[] = [vendorId];
...
export const updateConsultation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const consultation: any[] = await query(
      "SELECT * FROM vendor_consultations WHERE id = ? AND vendor_id = ?",
      [id, vendorId]
    );
    ...
    if (booking_status === 'completed' && oldConsultation.booking_status !== 'completed') {
      await sendReviewEmail(parseInt(id as string), client_email || oldConsultation.client_email);
    }
...
export const getMyEarnings = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = (req as any).user.id;
...
const sendReviewEmail = async (consultationId: number, clientEmail: string): Promise<void> => {
  try {
    ...
    const consultationDetails: any[] = await query(`
...
export const submitReview = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Verify token
    const tokenData: any[] = await query(`
    ...
    const existingReview: any[] = await query(
      "SELECT id FROM vendor_reviews WHERE consultation_id = ?",
      [tokenInfo.consultation_id]
    );
...
export const getReviewForm = async (req: Request, res: Response): Promise<void> => {
  try {
    const { token } = req.params;
...
export const testEmail = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await EmailService.sendTestEmail(
      email, 
      'Test Email from Vivaaha Consultation System'
    );
