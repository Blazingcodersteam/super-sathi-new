...
class GoogleAnalyticsController {
    ...
    static async updateSettings(req, res) {
        try {
            const { is_active, analytics_key } = req.body;
            // Validation
            ...
            const currentSettings = await query("SELECT id FROM google_analytics_settings WHERE status = 'active' ORDER BY created_at DESC LIMIT 1");
            let settingsId;
        ...
        `;
            ...
            const updatedSettings = await query("SELECT * FROM google_analytics_settings WHERE id = ?", [settingsId]);
            res.json({
                success: true,
    ...
    static async toggleActivation(req, res) {
        try {
            // Get current settings
            const currentSettings = await query("SELECT id, is_active FROM google_analytics_settings WHERE status = 'active' ORDER BY created_at DESC LIMIT 1");
            if (currentSettings.length === 0) {
            ...
            await query("UPDATE google_analytics_settings SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [newStatus, currentSettings[0].id]);
            // Get updated settings
            const updatedSettings = await query("SELECT * FROM google_analytics_settings WHERE id = ?", [currentSettings[0].id]);
            res.json({
                success: true,
    ...
    static async resetSettings(req, res) {
        try {
            ...
            await query("UPDATE google_analytics_settings SET is_active = 0, analytics_key = '', updated_at = CURRENT_TIMESTAMP WHERE id = ?", [currentSettings[0].id]);
            // Get updated settings
            const updatedSettings = await query("SELECT * FROM google_analytics_settings WHERE id = ?", [currentSettings[0].id]);
            res.json({
                success: true,
    ...
    `;
        const result = await query(insertQuery);
        const newSettings = await query("SELECT * FROM google_analytics_settings WHERE id = ?", [result.insertId]);
