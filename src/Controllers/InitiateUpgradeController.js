...
exports.initiateUpgrade = void 0;
...
const initiateUpgrade = async (req, res) => {
    ...
    try {
        ...
        const currentSub = await query("SELECT vs.*, vsp.monthly_price as current_price FROM vendor_subscriptions vs JOIN vendor_subscription_plans vsp ON vs.plan_id = vsp.id WHERE vs.vendor_id = ?", [vendorId]);
        if (currentSub.length === 0) {
            res.status(404).json({ success: false, message: "No active subscription found" });
            return;
        }
        // Get new plan details
        const newPlan = await query("SELECT * FROM vendor_subscription_plans WHERE id = ? AND is_active = 1", [new_plan_id]);
        if (newPlan.length === 0) {
        ...
        // Calculate proration
        const daysRemaining = Math.ceil((new Date(current.subscription_end_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
    ...
    `;
        ...
        if (payment_method === 'razorpay') {
            ...
            await query("UPDATE vendor_payments SET gateway_order_id = ? WHERE payment_id = ?", [razorpayOrder.id, paymentId]);
            res.json({
                success: true,
...
//# sourceMappingURL=InitiateUpgradeController.js.map