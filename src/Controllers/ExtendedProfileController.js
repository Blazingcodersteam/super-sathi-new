...
exports.updateLocationOLD = updateLocationOLD;
exports.updateLocation = updateLocation;
exports.updateHobbies = updateHobbies;
exports.updateBasicOld = updateBasicOld;
exports.updateAboutOld = updateAboutOld;
exports.updateAstroOld = updateAstroOld;
exports.updateFamilyOld = updateFamilyOld;
exports.updateCareerOld = updateCareerOld;
exports.updateLocationOld = updateLocationOld;
exports.updateHobbiesOld = updateHobbiesOld;
...
async function getCompleteProfile(req, res) {
    try {
        ...
        const [profile] = await query(`SELECT u.vivaaha_user_id, u.email, up.*, bg.blood_group, dis.disability_name, hi.health_condition,
...
async function updateLocationOLD(req, res) {
    try {
        const userId = req.user.user_id;
        const b = req.body;
        const fields = definedFields({
...
async function updateLocation(req, res) {
    try {
        const userId = req.user.user_id;
        const b = req.body;
        const { current_residence, residency_status, state_living_in, state_id, city_id, city, country_id, zip_code, pincode, latitude, longitude, grew_up_in, ethnic_origin_id } = b;
        ...
        if (Object.keys(addressFields).length > 0) {
            ...
            const [address] = await query("SELECT id FROM address_details WHERE user_id = ? AND address_type = 'current' LIMIT 1", [userId]);
            if (address) {
                await query(`UPDATE address_details SET ${cols.map((col) => `${col} = ?`).join(", ")} WHERE id = ?`, [...cols.map((col) => addressFields[col]), address.id]);
            }
            else {
                const insertFields = Object.assign({ address_type: 'current' }, addressFields);
                ...
                await query(`INSERT INTO address_details (user_id, ${insertCols.join(", ")}) VALUES (?, ${insertCols.map(() => "?").join(", ")})`, [userId, ...insertCols.map((col) => insertFields[col])]);
            }
        }
        const careerFields = definedFields({
...
async function updateBasicOld(req, res) {
    try {
        const userId = req.user.user_id;
...
async function updateAboutOld(req, res) {
    try {
        const userId = req.user.user_id;
...
async function updateAstroOld(req, res) {
    try {
        const userId = req.user.user_id;
...
async function updateFamilyOld(req, res) {
    try {
        const userId = req.user.user_id;
...
async function updateCareerOld(req, res) {
    try {
        const userId = req.user.user_id;
...
// Update Hobbies
async function updateHobbiesOld(req, res) {
    try {
        const userId = req.user.user_id;
...
//# sourceMappingURL=ExtendedProfileController.js.map