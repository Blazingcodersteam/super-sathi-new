 ...
 */
export async function getPostPurchaseAddons(req, res) {
  try {
...
export async function getFeatureUsage(req, res) {
  try {
    ...
    const contactTotal = parseInt(contactLimit[0]?.feature_value || '0');
    const boostTotal = parseInt(boostLimit[0]?.feature_value || '0');
    const contactUsed = Math.min(parseInt(contactUsageResult[0]?.usage_count || 0), contactTotal);
    const boostUsed = Math.min(parseInt(boostUsageResult[0]?.usage_count || 0), boostTotal);
 ...
 */
export async function purchaseVerificationBadge(req, res) {
  try {
    const userId = req.user.user_id;
 ...
 */
export async function getVerificationBadgeStatus(req, res) {
  try {
    const userId = req.user.user_id;
 ...
 */
export async function confirmMarriage(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function changePlan(req, res) {
  try {
    ...
    const gstCalculation = await GSTHelper.calculateGST(finalAmount, userLocation?.state_id);
 ...
 */
export async function activateFreeSubscription(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    await query(
      ...
      [invoiceNumber, paymentResult.insertId]
    );
