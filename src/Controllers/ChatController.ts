...
import { getIO, getUserSocketStatus } from "../socket/SocketManager";
...
async function sendOfflineChatPushNotification(receiverId: number, payload: any) {
  ...
  try {
    ...
    const senderName = payload.sender?.name || 'New message';
    ...
    const firebaseMessageId = await sendPushNotification({
      ...
      data: {
        ...
        sender_photo: String(payload.sender?.photo || ''),
      },
    });
...
export async function getChatListOLD(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function getMessages(req, res) {
  try {
    ...
    const ensureFullUrl = (pic: string | null) => {
      if (!pic) return null;
...
export async function sendMessage(req, res) {
  try {
    ...
    const socketPayload = {
      ...
      sender: {
        id: userId,
        name: `${senderProfile?.first_name || ''} ${senderProfile?.last_name || ''}`.trim() || 'Unknown',
        photo: senderProfile?.profile_picture || null
      },
