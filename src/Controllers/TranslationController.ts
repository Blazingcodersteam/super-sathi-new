...
async function getLanguageCodes(): Promise<string[]> {
  const langs = await query(
    "SELECT language_code FROM language_master WHERE is_active = 1 ORDER BY id ASC"
  );
  return langs.map((l: any) => l.language_code);
}

// Helper: get all languages with details
async function getAllLanguages(): Promise<any[]> {
  const langs = await query(
    "SELECT id, language_name, language_code, is_active FROM language_master ORDER BY id ASC"
  );
  return langs;
}
...
function isValidTranslation(translated: string): boolean {
  ...
  if (/[\u00c0-\u00ff]{3,}/.test(translated)) return false;
  return true;
}

function translateText(text: string, targetLang: string): Promise<string | null> {
...
export const languageMaster = {
  ...
  async getAll(req, res) {
    try {
      const languages = await query(
        "SELECT * FROM language_master ORDER BY id ASC"
      );
  ...
  async create(req, res) {
    try {
      ...
      const [existing] = await query(
        "SELECT id FROM language_master WHERE language_code = ?", [code]
      );
      if (existing) {
      ...
      await query(
        ...
        [language_name, code, is_active]
      );

      // Dynamically add column to translations table if not exists
      await query(
        `ALTER TABLE translations ADD COLUMN IF NOT EXISTS \`${code}\` text DEFAULT NULL`
      );
  ...
  async update(req, res) {
    try {
      ...
      const [existing] = await query(
        "SELECT id FROM language_master WHERE id = ?", [id]
      );
      if (!existing) {
      ...
      if (language_name !== undefined) { updates.push("language_name = ?"); values.push(language_name); }
  ...
  async delete(req, res) {
    try {
      ...
      const [existing] = await query(
        "SELECT id FROM language_master WHERE id = ? AND is_active = 1", [id]
      );
      if (!existing) {
        return res.status(404).json({ success: false, message: "Language not found" });
      }
...
export const translations = {
  ...
  async getAll(req, res) {
    try {
      ...
      const rows = await query(
        `SELECT id, text_key, actual_text, platform, status, language_data, auto_converted, created_at, updated_at FROM translations ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
        [...params, Number(limit), offset]
      );

      const [{ total }] = await query(
        `SELECT COUNT(*) as total FROM translations ${where}`,
        params
      );
  ...
  async getById(req, res) {
    try {
      ...
      const [row] = await query(
        "SELECT id, text_key, actual_text, platform, status, language_data, auto_converted, created_at, updated_at FROM translations WHERE id = ? AND status = 1", 
        [id]
      );
      if (!row) {
  ...
  async getByKey(req, res) {
    try {
      ...
      const [row] = await query(
        `SELECT id, text_key, actual_text, platform, status, language_data, auto_converted, created_at, updated_at FROM translations ${where} LIMIT 1`, 
        params
      );
      if (!row) {
  ...
  async create(req, res) {
    try {
      const { text_key, actual_text, platform = "both", status = 1, auto_conversion = false, ...langValues } = req.body;
      ...
      const [existing] = await query(
        "SELECT id, language_data FROM translations WHERE text_key = ?",
        [text_key]
      );

      // Get all active language codes from language_master
      const langCodes = await getLanguageCodes();
      const languageData: Record<string, string | null> = {};
      ...
      if (existing) {
        ...
        await query(
          `UPDATE translations SET actual_text = ?, platform = ?, status = ?, language_data = ?, auto_converted = ?, updated_at = NOW() WHERE text_key = ?`,
          [actual_text, platform, status, JSON.stringify(languageData), auto_conversion ? 1 : 0, text_key]
        );
        ...
        const result = await query(
          `INSERT INTO translations (text_key, actual_text, platform, status, language_data, auto_converted) VALUES (?, ?, ?, ?, ?, ?)`,
          [text_key, actual_text, platform, status, JSON.stringify(languageData), auto_conversion ? 1 : 0]
        );
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const { actual_text, platform, status, text_key, auto_conversion = false, ...langValues } = req.body;

      const [existing] = await query(
        "SELECT id, language_data, actual_text, text_key FROM translations WHERE id = ?", 
        [id]
      );
      if (!existing) {
        return res.status(404).json({ success: false, message: "Translation not found" });
      ...
      if (text_key !== undefined)    { updates.push("text_key = ?");    values.push(text_key); }
      if (actual_text !== undefined) { updates.push("actual_text = ?"); values.push(actual_text); }
      ...
      let languageData: Record<string, string | null> = {};
      ...
      const [updated] = await query(
        "SELECT id, text_key, actual_text, platform, status, language_data, auto_converted, created_at, updated_at FROM translations WHERE id = ?", 
        [id]
      );
      ...
      const processedRow = {
        ...updated,
        language_data: (() => {
          if (!updated.language_data) return {};
          try {
  ...
  async exportByLanguage(req, res) {
    try {
      ...
      const [langExists] = await query(
        "SELECT id FROM language_master WHERE language_code = ? AND is_active = 1", [lang]
      );
      if (!langExists) {
      ...
      const rows = await query(
        `SELECT text_key, language_data FROM translations WHERE status = 1 AND (platform = ? OR platform = 'both') ORDER BY text_key ASC`,
        [lang, platform]
      );
