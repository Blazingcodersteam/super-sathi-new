...
export const gstTypeMaster = {
  ...
  async getAll(req, res) {
    try {
      ...
      try {
        await query(
          `ALTER TABLE gst_type_master ADD COLUMN tax_percentage DECIMAL(5,2) DEFAULT 0.00 COMMENT 'Tax percentage for this GST type'`
        );
      } catch (alterError) {
      ...
      const records = await query(
        `SELECT id, gst_type_name, gst_type_code, description, 
         ...
         FROM gst_type_master ${whereClause} ORDER BY id DESC LIMIT ? OFFSET ?`,
        [...params, parseInt(limit), offset]
      );

      const [{ total }] = await query(
        `SELECT COUNT(*) as total FROM gst_type_master ${whereClause}`,
        params
      );
  ...
  async getById(req, res) {
    try {
      ...
      const [record] = await query(
        `SELECT id, gst_type_name, gst_type_code, description, 
         COALESCE(tax_percentage, 0.00) as tax_percentage, is_active, created_at, updated_at 
         FROM gst_type_master WHERE id = ? AND is_active = 1`, 
        [id]
      );
  ...
  async create(req, res) {
    try {
      ...
      const [existing] = await query(
        `SELECT id FROM gst_type_master WHERE gst_type_code = ?`,
        [gst_type_code]
      );
      ...
      const result = await query(
        `INSERT INTO gst_type_master (gst_type_name, gst_type_code, description, tax_percentage, is_active) VALUES (?, ?, ?, ?, ?)`,
        [gst_type_name, gst_type_code, description, tax_percentage, is_active]
      );
    ...
    try {
      ...
      if (gst_type_code) {
        const [duplicate] = await query(
          `SELECT id FROM gst_type_master WHERE gst_type_code = ? AND id != ?`,
          [gst_type_code, id]
        );
      ...
      await query(
        `UPDATE gst_type_master SET ${updateFields.join(", ")} WHERE id = ?`,
        [...updateValues, id]
      );
