...
function resolveFeatureName(code) {
...
async function checkFeatureAccess(req, res) {
    ...
    `, [userId, dbFeatureName]);
        const used = parseInt((usage === null || usage === void 0 ? void 0 : usage.usage_count) || 0);
 ...
 */
async function useFeature(req, res) {
    try {
        const userId = req.user.user_id;
 ...
 */
async function getPlanComparison(req, res) {
    try {
 ...
 */
async function getMarriageConfirmationStatus(req, res) {
    try {
        const userId = req.user.user_id;
