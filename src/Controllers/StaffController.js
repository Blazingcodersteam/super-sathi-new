...
exports.staffManagement = exports.staffAuth = exports.uploadStaffImage = void 0;
...
exports.uploadStaffImage = multer({
...
exports.staffAuth = {
    ...
    async register(req, res) {
        try {
            const { first_name, last_name, middle_name, mobile_number, email, password, role_id } = req.body;
            // Basic validation
            ...
            const [existingEmail] = await query('SELECT id FROM staff WHERE email = ?', [email]);
            if (existingEmail) {
                res.status(409).json({
                    success: false,
            ...
            const [existingMobile] = await query('SELECT id FROM staff WHERE mobile_number = ?', [mobile_number]);
            if (existingMobile) {
                res.status(409).json({
                    success: false,
            ...
            const [role] = await query('SELECT id FROM user_type_master WHERE id = ? AND id NOT IN (1,7) AND is_active = 1', [parseInt(role_id)]);
            if (!role) {
                res.status(400).json({
                    success: false,
            ...
            // Insert staff member
            const result = await query(`INSERT INTO staff (
    ...
    async login(req, res) {
        ...
        try {
            ...
            const [staff] = await query(`SELECT s.*, utm.type_name as role_name, utm.description as role_description,
                utm.permissions_version
         FROM staff s
         LEFT JOIN user_type_master utm ON s.role_id = utm.id
         WHERE s.email = ? AND s.status = 'active'`, [email]);
            if (!staff) {
                res.status(401).json({
                    success: false,
         ...
         ORDER BY mm.parent_id ASC, mm.sort_order ASC`, [staff.role_id]);
            ...
            const token = jwt.sign({
                ...
                permissions_version: (_a = staff.permissions_version) !== null && _a !== void 0 ? _a : 0
            }, process.env.JWT_SECRET_KEY, { expiresIn: '24h' });
            // Remove password from response
            const { password: _ } = staff, staffData = __rest(staff, ["password"]);
            res.status(200).json({
                success: true,
    ...
    async getProfile(req, res) {
        var _a;
        try {
            const staffId = (_a = req.staff) === null || _a === void 0 ? void 0 : _a.staff_id;
            const [staff] = await query(`SELECT s.*, utm.type_name as role_name, utm.description as role_description
         FROM staff s
         LEFT JOIN user_type_master utm ON s.role_id = utm.id
         WHERE s.id = ?`, [staffId]);
            ...
            // Remove password from response
            const { password } = staff, staffData = __rest(staff, ["password"]);
            res.status(200).json({
                success: true,
    ...
    async updateProfile(req, res) {
        ...
        try {
            const staffId = (_a = req.staff) === null || _a === void 0 ? void 0 : _a.staff_id;
            const { first_name, last_name, middle_name, mobile_number, email } = req.body;
            // Get current staff data
            const [existingStaff] = await query('SELECT * FROM staff WHERE id = ?', [staffId]);
            if (!existingStaff) {
                res.status(404).json({
                    success: false,
                    message: 'Staff profile not found'
            ...
            if (mobile_number && mobile_number !== existingStaff.mobile_number) {
                const [mobileExists] = await query('SELECT id FROM staff WHERE mobile_number = ? AND id != ?', [mobile_number, staffId]);
                if (mobileExists) {
                    res.status(409).json({
                        success: false,
                        message: 'Mobile number already exists'
            ...
            const [updatedStaff] = await query(`SELECT s.*, utm.type_name as role_name, utm.description as role_description
         FROM staff s
         LEFT JOIN user_type_master utm ON s.role_id = utm.id
         WHERE s.id = ?`, [staffId]);
            // Remove password from response
            const { password } = updatedStaff, staffData = __rest(updatedStaff, ["password"]);
            res.status(200).json({
                success: true,
                message: 'Profile updated successfully',
                data: Object.assign(Object.assign({}, staffData), { image_updated: !!req.file })
            });
        }
        catch (error) {
    ...
    async changePassword(req, res) {
        ...
        try {
            ...
            // Get current staff data
            const [staff] = await query('SELECT id, password FROM staff WHERE id = ?', [staffId]);
            if (!staff) {
                res.status(404).json({
                    success: false,
            ...
            // Update password
            await query('UPDATE staff SET password = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hashedNewPassword, staffId]);
            res.status(200).json({
                success: true,
    ...
    async getAvailableRoles(req, res) {
        try {
            const roles = await query(`SELECT id, type_name as role_name, description
    ...
    async getMenuPermissions(req, res) {
        ...
        try {
            const staffId = (_a = req.staff) === null || _a === void 0 ? void 0 : _a.staff_id;
            const roleId = (_b = req.staff) === null || _b === void 0 ? void 0 : _b.role_id;
            if (!roleId) {
                res.status(400).json({
                    success: false,
         ...
         INNER JOIN role_menu_permissions rmp ON mm.id = rmp.menu_id
         WHERE rmp.role_id = ? AND mm.is_active = 1 AND rmp.can_view = 1
         ORDER BY mm.parent_id ASC, mm.sort_order ASC`, [roleId]);
            // Organize menus in hierarchical structure
            const organizeMenus = (menus) => {
                const menuMap = new Map();
                const rootMenus = [];
...
exports.staffManagement = {
    ...
    async getAll(req, res) {
        try {
            ...
            // Get total count
            const [{ total }] = await query(`SELECT COUNT(*) as total FROM staff s ${whereClause}`, queryParams);
         ...
         LIMIT ? OFFSET ?`, [...queryParams, limitNum, offset]);
            // Remove passwords from response
            const staffWithoutPasswords = staff.map((_a) => {
                var { password } = _a, staffMember = __rest(_a, ["password"]);
                return staffMember;
            });
            res.status(200).json({
                success: true,
    ...
    async getById(req, res) {
        try {
         ...
         WHERE s.id = ?`, [staffId]);
            ...
            // Remove password from response
            const { password } = staff, staffWithoutPassword = __rest(staff, ["password"]);
            res.status(200).json({
                success: true,
    ...
    async getByRoleId(req, res) {
        try {
         ...
         LEFT JOIN user_type_master utm ON s.role_id = utm.id
         WHERE s.role_id = ?
         ORDER BY s.created_at DESC`, [roleIdNum]);
            // Remove passwords from response
            const staffWithoutPasswords = staff.map((_a) => {
                var { password } = _a, staffMember = __rest(_a, ["password"]);
                return staffMember;
    ...
    async create(req, res) {
        ...
        try {
            ...
            const data = req.body;
            const { first_name, last_name, middle_name, mobile_number, email, password, role_id, status = 'active' } = data;
            // Basic validation
            if (!first_name || !last_name || !mobile_number || !email || !password || !role_id) {
                res.status(400).json({
                    success: false,
            ...
            const adminId = ((_a = req.admin) === null || _a === void 0 ? void 0 : _a.user_id) || ((_b = req.user) === null || _b === void 0 ? void 0 : _b.id);
            // Check if email already exists
            const [existingEmail] = await query('SELECT id FROM staff WHERE email = ?', [email]);
            if (existingEmail) {
                res.status(409).json({
            ...
            // Validate role (exclude admin=2, user=1, vendor=7)
            const [role] = await query('SELECT id FROM user_type_master WHERE id = ? AND id NOT IN (1, 2, 7) AND is_active = 1', [parseInt(role_id)]);
            if (!role) {
                res.status(400).json({
                    success: false,
                    message: 'Invalid role ID or role not available for staff'
    ...
    async update(req, res) {
        ...
        try {
            const { id } = req.params;
            const staffId = parseInt(id);
            const { first_name, last_name, middle_name, mobile_number, email, password, role_id, status } = req.body;
            ...
            if (role_id) {
                const [role] = await query('SELECT id FROM user_type_master WHERE id = ? AND id NOT IN (1, 2, 7) AND is_active = 1', [role_id]);
                if (!role) {
                    res.status(400).json({
                        success: false,
                        message: 'Invalid role ID or role not available for staff'
    ...
    async delete(req, res) {
        ...
        try {
            ...
            // Check if staff exists
            const [existingStaff] = await query('SELECT id, first_name, last_name, status FROM staff WHERE id = ?', [staffId]);
            if (!existingStaff) {
                res.status(404).json({
                    success: false,
                    message: 'Staff member not found'
            ...
            const result = await query(`UPDATE staff SET
          status = 'inactive',
          updated_by = ?,
          updated_at = CURRENT_TIMESTAMP
        WHERE id = ?`, [adminId, staffId]);
            if (result.affectedRows === 0) {
                res.status(400).json({
                    success: false,
    ...
    async changeStatus(req, res) {
        ...
        try {
          ...
          updated_by = ?,
          updated_at = CURRENT_TIMESTAMP
        WHERE id = ?`, [status, adminId, staffId]);
            if (result.affectedRows === 0) {
                res.status(400).json({
                    success: false,
    ...
    async getAvailableRoles(req, res) {
        try {
            const roles = await query(`SELECT id, type_name as role_name, description, is_active as status
         FROM user_type_master
         WHERE id NOT IN (1, 2, 7) AND is_active = 1
         ORDER BY type_name ASC`);
            res.status(200).json({
    ...
    async assignRole(req, res) {
        ...
        try {
            ...
            await query("UPDATE staff SET role_id = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [role_id, adminId, userId]);
