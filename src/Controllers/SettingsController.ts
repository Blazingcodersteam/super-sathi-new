...
export async function sendPhoneUpdateOTP(req, res) {
  try {
    ...
    await query(
      "INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at) VALUES (?, ?, 'phone', ?, ?) ON DUPLICATE KEY UPDATE otp_code = VALUES(otp_code), contact_info = VALUES(contact_info), expires_at = VALUES(expires_at), created_at = NOW()",
      [userId, otp, new_phone, expiresAt]
    );
    ...
    try {
      await EmailService.sendTemplateEmail(
        'phone_update_otp',
        user.email,
        { user_name: user.first_name || user.email, otp, new_phone },
        {
...
export async function updatePhoneWithOTP(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    const [otpRecord] = await query(
      "SELECT otp_code, expires_at, contact_info FROM otp_verification WHERE user_id = ? AND otp_type = 'phone' AND expires_at > NOW() AND is_verified = FALSE ORDER BY created_at DESC LIMIT 1",
      [userId]
    );
    ...
    await query(
      "UPDATE otp_verification SET is_verified = TRUE, verified_at = NOW() WHERE user_id = ? AND otp_type = 'phone'",
      [userId]
    );
...
export async function sendEmailUpdateOTP(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    await query(
      "INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at) VALUES (?, ?, 'email_verification', ?, ?) ON DUPLICATE KEY UPDATE otp_code = VALUES(otp_code), contact_info = VALUES(contact_info), expires_at = VALUES(expires_at), created_at = NOW()",
      [userId, otp, new_email, expiresAt]
    );
    ...
    try {
      await EmailService.sendTemplateEmail(
        'email_update_otp',
        user.email,
        { user_name: user.first_name || user.email, otp, new_email },
        {
...
export async function updateEmailWithOTP(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    // Verify OTP
    const [otpRecord] = await query(
      "SELECT otp_code, expires_at, contact_info FROM otp_verification WHERE user_id = ? AND otp_type = 'email_verification' AND expires_at > NOW() AND is_verified = FALSE ORDER BY created_at DESC LIMIT 1",
      [userId]
    );
    ...
    // Mark OTP as verified
    await query(
      "UPDATE otp_verification SET is_verified = TRUE, verified_at = NOW() WHERE user_id = ? AND otp_type = 'email_verification'",
      [userId]
    );
...
export async function getUserSettings(req, res) {
  try {
    ...
    const [profile] = await query(
      `SELECT up.*, u.email, u.phone, bg.blood_group, dis.disability_name, hi.health_condition,
    ...
    const [contactFilterSettings] = await query(
      "SELECT * FROM contact_filter_settings WHERE user_id = ?",
      [userId]
    );

    const [emailSmsSettings] = await query(
      "SELECT * FROM email_sms_settings WHERE user_id = ?",
      [userId]
    );
    ...
    const [notificationSettings] = await query(
      "SELECT * FROM notification_settings WHERE user_id = ?",
      [userId]
    );

    const [accountSettings] = await query(
      "SELECT * FROM account_settings WHERE user_id = ?",
      [userId]
    );

    // Get user's selected message template IDs
    const [messageSettings] = await query(
      "SELECT * FROM user_message_settings WHERE user_id = ?",
      [userId]
    );
    ...
    if (existing) {
      await query(
        `UPDATE user_settings SET
         contact_display_status = ?
         WHERE user_id = ?`,
        [contact_display_status, userId]
      );
    } else {
      await query(
        `INSERT INTO user_settings (user_id, contact_display_status)
         VALUES (?, ?)`,
        [userId, contact_display_status]
      );
    }
...
export async function updatePrivacySettings(req, res) {
  try {
    ...
    if (existing) {
      ...
      await query(
        `UPDATE privacy_settings SET ${updateFields.join(', ')} WHERE user_id = ?`,
        [...updateValues, userId]
      );
    } else {
      ...
      await query(
        `INSERT INTO privacy_settings (user_id, ${columns.join(', ')}) VALUES (?, ${columns.map(() => '?').join(', ')})`,
        [userId, ...updateValues]
      );
    }
    ...
    if (existing) {
      await query(
        `UPDATE notification_settings SET
         ...
         weekly_digest = ?
         WHERE user_id = ?`,
        [new_matches, profile_views, messages, interests_received, interests_accepted,
         photo_requests, contact_requests, horoscope_requests, membership_expiry,
         promotional_emails, weekly_digest, userId]
      );
    } else {
      await query(
        `INSERT INTO notification_settings (user_id, new_matches, profile_views, messages,
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [userId, new_matches, profile_views, messages, interests_received, interests_accepted,
         photo_requests, contact_requests, horoscope_requests, membership_expiry,
    ...
    if (accountExists) {
      await query(
        `UPDATE account_settings SET
         ...
         WHERE user_id = ?`,
        [two_factor_enabled, login_alerts, session_timeout, userId]
      );
    } else {
      await query(
        `INSERT INTO account_settings (user_id, two_factor_enabled, login_alerts, session_timeout)
         VALUES (?, ?, ?, ?)`,
        [userId, two_factor_enabled, login_alerts, session_timeout]
      );
    }
...
export async function changePassword(req, res) {
  try {
    ...
    // Update account settings
    await query(
      `INSERT INTO account_settings (user_id, password_changed_at)
       VALUES (?, NOW())
...
export async function deactivateAccount(req, res) {
  try {
    ...
    // Update account settings
    await query(
      `INSERT INTO account_settings (user_id, account_deactivated, deactivation_reason, deactivated_at)
       VALUES (?, TRUE, ?, NOW())
       ON DUPLICATE KEY UPDATE
       account_deactivated = TRUE, deactivation_reason = ?, deactivated_at = NOW()`,
      [userId, reason, reason]
    );

    // Update user status
    await query(
      "UPDATE users SET status = 2 WHERE id = ?", // 2 = Inactive
      [userId]
    );
...
export async function deleteAccount(req, res) {
  try {
    ...
    const [reason] = await query(
      "SELECT * FROM delete_account_reasons_master WHERE id = ?",
      [reason_id]
    );
    ...
    await query(
      "INSERT INTO user_account_deletion (user_id, reason_id, additional_feedback) VALUES (?, ?, ?)",
      [userId, reason_id, additional_feedback || null]
    );

    // Free up email/phone so the user can re-register with the same credentials
    await query(
      "UPDATE users SET email = CONCAT(email, '_deleted_', id), phone = CASE WHEN phone IS NOT NULL THEN LEFT(CONCAT(phone, '_d', id), 20) ELSE NULL END, status = 4 WHERE id = ?",
      [userId]
    );
...
export async function blockUser(req, res) {
  try {
    ...
    const [existing] = await query(
      "SELECT block_users FROM privacy_settings WHERE user_id = ?",
      [userId]
    );
    ...
    if (action === 'block') {
      ...
      await query(
        `INSERT IGNORE INTO user_actions (user_id, target_user_id, action_type_id) VALUES (?, ?, 3)`,
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET block_users = ? WHERE user_id = ?",
        [JSON.stringify(blockedUsers), userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, block_users) VALUES (?, ?)",
...
export async function getBlockedUsers(req, res) {
  try {
    ...
    const blockedUsers = await query(
      `SELECT u.id, up.first_name, up.last_name, up.profile_picture
       FROM users u
       JOIN user_profiles up ON u.id = up.user_id
       WHERE u.id IN (${blockedUserIds.map(() => '?').join(',')})`,
      blockedUserIds
    );
...
export async function updatePhotoPassword(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET photo_password_protected = ?, photo_password = ? WHERE user_id = ?",
        [enable_password_protection, photo_password, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, photo_password_protected, photo_password) VALUES (?, ?, ?)",
        [userId, enable_password_protection, photo_password]
      );
    }
...
export async function getProfileCompletion(req, res) {
  try {
    const userId = req.user.user_id;

    const [accountSettings] = await query(
      "SELECT profile_completion_percentage, mandatory_fields_completed FROM account_settings WHERE user_id = ?",
      [userId]
    );
...
export async function updateShortlistSetting(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET shortlist_visibility = ? WHERE user_id = ?",
        [shortlist_visibility, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, shortlist_visibility) VALUES (?, ?)",
        [userId, shortlist_visibility]
      );
    }
...
export async function updateDoNotDisturbSetting(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET allow_premium_calls = ?, do_not_disturb = ? WHERE user_id = ?",
        [allow_premium_calls, do_not_disturb, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, allow_premium_calls, do_not_disturb) VALUES (?, ?, ?)",
        [userId, allow_premium_calls, do_not_disturb]
      );
    }
...
export async function updateProfilePrivacySetting(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET profile_visibility = ? WHERE user_id = ?",
        [profile_visibility, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, profile_visibility) VALUES (?, ?)",
        [userId, profile_visibility]
      );
    }
...
export async function getHideProfileDurations(req, res) {
  try {
    const durations = await query(
      "SELECT * FROM hide_profile_duration_master ORDER BY duration_days ASC"
    );
...
export async function hideProfile(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    const [duration] = await query(
      "SELECT * FROM hide_profile_duration_master WHERE id = ?",
      [duration_id]
    );
    ...
    const [existing] = await query(
      "SELECT * FROM user_hide_profile WHERE user_id = ? AND is_active = TRUE AND hide_end_date > NOW()",
      [userId]
    );
    ...
    await query(
      "UPDATE user_hide_profile SET is_active = FALSE WHERE user_id = ?",
      [userId]
    );

    // Upsert — reuse existing row if one exists, otherwise insert
    const [anyRow] = await query(
      "SELECT id FROM user_hide_profile WHERE user_id = ? ORDER BY id DESC LIMIT 1",
      [userId]
    );
    ...
    if (anyRow) {
      ...
      await query(
        "UPDATE user_hide_profile SET duration_id = ?, hide_end_date = ?, is_active = TRUE, hide_start_date = NOW(), updated_at = NOW() WHERE id = ?",
        [duration_id, hideEndDate, anyRow.id]
      );
    } else {
      // First time — insert
      await query(
        "INSERT INTO user_hide_profile (user_id, duration_id, hide_end_date) VALUES (?, ?, ?)",
        [userId, duration_id, hideEndDate]
      );
    }
...
// Unhide Profile
export async function unhideProfile(req, res) {
  try {
    const userId = req.user.user_id;

    // Deactivate current hide profile
    await query(
      "UPDATE user_hide_profile SET is_active = FALSE WHERE user_id = ? AND is_active = TRUE",
      [userId]
    );
...
export async function getHideProfileStatus(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function deleteProfile(req, res) {
  try {
    ...
    await query(
      `INSERT INTO account_settings (user_id, profile_deleted)
       VALUES (?, TRUE)
       ON DUPLICATE KEY UPDATE profile_deleted = TRUE`,
      [userId]
    );
...
export async function updateContactSettings(req, res) {
  try {
    ...
    if (existing) {
      await query(
        "UPDATE user_settings SET contact_display_status = ? WHERE user_id = ?",
        [contact_display_status, userId]
      );
    } else {
      await query(
        "INSERT INTO user_settings (user_id, contact_display_status) VALUES (?, ?)",
        [userId, contact_display_status]
      );
    }
...
export async function updateContactFilterSettings(req, res) {
  try {
    const userId = req.user.user_id;
    const {
      age_min,
    ...
    const [existing] = await query(
      "SELECT id FROM contact_filter_settings WHERE user_id = ?",
      [userId]
    );

    if (existing) {
      await query(
        `UPDATE contact_filter_settings SET
      ...
      await query(
        `INSERT INTO contact_filter_settings (user_id, age_min, age_max, height_min, height_max,
         country_filter, religion_filter, community_filter, mother_tongue_filter, marital_status_filter)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [userId, age_min, age_max, height_min, height_max,
         JSON.stringify(country_filter || []), JSON.stringify(religion_filter || []),
         JSON.stringify(community_filter || []), JSON.stringify(mother_tongue_filter || []),
...
export async function updateEmailSmsAlertSettings(req, res) {
  try {
    const userId = req.user.user_id;
    const {
      match_mail_frequency,
      send_broader_matches,
      premium_match_mail,
      recent_visitors_email,
      shortlisted_email,
      viewed_profiles_email,
      similar_profiles_email,
      contact_alert,
      message_received_alert,
      sms_alert,
      profile_blaster,
      vivaaha_specials,
      vivaaha_insite
    ...
    const [existing] = await query(
      "SELECT id FROM email_sms_settings WHERE user_id = ?",
      [userId]
    );

    if (existing) {
      await query(
        `UPDATE email_sms_settings SET
      ...
      await query(
        `INSERT INTO email_sms_settings (user_id, match_mail_frequency, send_broader_matches,
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [userId, match_mail_frequency, send_broader_matches, premium_match_mail,
         recent_visitors_email, shortlisted_email, viewed_profiles_email,
         similar_profiles_email, contact_alert, message_received_alert,
         sms_alert, profile_blaster, vivaaha_specials, vivaaha_insite]
      );
...
export async function getMessageTemplates(req, res) {
  try {
    ...
    const templates = await query(
      `SELECT * FROM message_templates ORDER BY template_type, id`
    );
    ...
    const resolvedTemplates = templates.map(t => ({
      ...t,
      message_text: t.message_text?.replace(/\{mobile\}/g, userPhone)
    }));
...
export async function updateMessageSettings(req, res) {
  try {
    ...
    const [existing] = await query(
      "SELECT id FROM user_message_settings WHERE user_id = ?",
      [userId]
    );

    if (existing) {
      await query(
        `UPDATE user_message_settings SET
         connect_message_id = ?, accept_message_id = ?, remind_message_id = ?
         WHERE user_id = ?`,
        [connect_message_id, accept_message_id, remind_message_id, userId]
      );
    } else {
      await query(
        `INSERT INTO user_message_settings (user_id, connect_message_id, accept_message_id, remind_message_id)
         VALUES (?, ?, ?, ?)`,
        [userId, connect_message_id, accept_message_id, remind_message_id]
      );
    }
...
export async function updateIncomePrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET income_privacy = ? WHERE user_id = ?",
        [income_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, income_privacy) VALUES (?, ?)",
        [userId, income_privacy]
      );
    }
...
export async function updateDobPrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET dob_privacy = ? WHERE user_id = ?",
        [dob_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, dob_privacy) VALUES (?, ?)",
        [userId, dob_privacy]
      );
    }
...
export async function updateContactFilterPrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET contact_filter_privacy = ? WHERE user_id = ?",
        [contact_filter_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, contact_filter_privacy) VALUES (?, ?)",
        [userId, contact_filter_privacy]
      );
    }
...
export async function updateEmailPrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET email_privacy = ? WHERE user_id = ?",
        [email_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, email_privacy) VALUES (?, ?)",
        [userId, email_privacy]
      );
    }
...
export async function updatePhonePrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET phone_privacy = ? WHERE user_id = ?",
        [phone_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, phone_privacy) VALUES (?, ?)",
        [userId, phone_privacy]
      );
    }
...
export async function updatePhotoPrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      ...
      await query(
        "INSERT INTO privacy_settings (user_id, profile_photo_privacy, album_photo_privacy) VALUES (?, ?, ?)",
        [userId, profile_photo_privacy || 'visible_to_all', album_photo_privacy || 'visible_to_all']
      );
    }
...
export async function updateContactFilterSettingsWithIds(req, res) {
  try {
    const userId = req.user.user_id;
    const {
      age_min,
...
export async function updateDisplayNamePrivacy(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existing) {
      await query(
        "UPDATE privacy_settings SET display_name_privacy = ? WHERE user_id = ?",
        [display_name_privacy, userId]
      );
    } else {
      await query(
        "INSERT INTO privacy_settings (user_id, display_name_privacy) VALUES (?, ?)",
        [userId, display_name_privacy]
      );
    }
