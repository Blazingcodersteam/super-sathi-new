...
exports.getStateListByCountryID = getStateListByCountryID;
...
async function getStateListByCountryID(req, res) {
    try {
        ...
        const states_list = await query(`SELECT * FROM states WHERE country_id = ?`, [country_id]);
        return res.status(200).json({
            success: true,
            states_list,
        });
