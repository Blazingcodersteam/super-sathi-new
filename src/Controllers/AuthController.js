...
exports.beforeOTP = beforeOTP;
exports.afterotp = afterotp;
const utils = require("util");
const jwt = require("jsonwebtoken");
const bcrypt = require("bcrypt");
const SMSService_1 = require("./SMSService");
const MatchActionsController_1 = require("./MatchActionsController");
...
async function sendOTP(req, res) {
    try {
        ...
        const [user] = await query("SELECT id, phone FROM users WHERE phone = ? AND status != 4", [phone]);
        if (!user) {
            return res.status(404).json({
                success: false,
                message: "User not found",
        ...
        await query("UPDATE users SET otp = ?, otp_expires_at = ? WHERE id = ?", [otp, expiresAt, user.id]);
        // Deliver the OTP over SMS in real time.
        const sms = await (0, SMSService_1.sendOtpSms)(user.phone, otp);
        // if (!sms.success) {
...
async function loginWithPassword(req, res) {
    try {
        ...
        const [user] = await query(`SELECT u.id, u.email, u.password, u.status FROM users u WHERE u.email = ?`, [email]);
        if (!user) {
            return res.status(404).json({
                success: false,
        ...
        const token = jwt.sign({ user_id: user.id, email: user.email, user_type: "user" }, JWT_SECRET, { expiresIn: "30d" });
...
async function loginWithOTP(req, res) {
    try {
        ...
        const [user] = await query(`SELECT u.id, u.email, u.phone, u.status, u.otp, u.otp_expires_at FROM users u WHERE u.phone = ?`, [phone]);
        if (!user) {
            return res.status(401).json({
                success: false,
                message: "Invalid credentials",
        ...
        // Clear OTP after successful login
        await query("UPDATE users SET otp = NULL, otp_expires_at = NULL WHERE id = ?", [user.id]);
        // Mark the user verified on OTP login (no separate KYC step)
        await query("UPDATE user_profiles SET aadhaar_verified = 1 WHERE user_id = ?", [user.id]);
        const token = jwt.sign({ user_id: user.id, email: user.email, user_type: "user" }, JWT_SECRET, { expiresIn: "30d" });
        const completeProfile = await getCompleteUserProfile(user.id);
        res.json({
            success: true,
 ...
 * the profile.
 */
async function beforeOTP(req, res) {
    try {
        ...
        // Persist OTP on the user record (users.otp / users.otp_expires_at).
        await query("UPDATE users SET otp = ?, otp_expires_at = ? WHERE id = ?", [otp, expiresAt, userId]);
...
async function afterotp(req, res) {
    var _a, _b;
    try {
