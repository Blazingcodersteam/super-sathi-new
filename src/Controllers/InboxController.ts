...
import { createPhotoRequestAlert } from "./AlertsController";
import { applyPrivacyFilterToMatches } from "../utils/privacyFilter";
...
async function getProfileDetails(userId: number, currentUserId: number) {
  ...
  const filtered = await import('../utils/privacyFilter').then(m => m.applyPrivacyFilter({
    ...profile,
    family: family || {},
    astro: astro || {},
    location: location || {},
...
export async function getRequests(req, res) {
  try {
    ...
    if (type === 'pending_requests') {
      ...
      total = (interestCount?.total || 0) + (connectCount?.total || 0);
      ...
      total = (sentInterestCount?.total || 0) + (sentConnectCount?.total || 0);
    }
...
// Send Photo Request
export async function sendPhotoRequest(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    if (existingRequest) {
      if (existingRequest.status === 'sent') {
        ...
        await createPhotoRequestAlert(targetUserId, userId);
...
export async function cancelPhotoRequest(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function getDeletedInterests(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function getInboxSummary(req, res) {
  try {
    ...
    const acceptedCount = { count: (acceptedInterestCount?.count || 0) + (acceptedConnectCount?.count || 0) };
    ...
    const sentTotal = (sentInterestsCount?.count || 0) + (sentConnectsCount?.count || 0);
    ...
    const summary = {
      ...
      requests: (pendingInterestsCount?.count || 0) + (pendingConnectsCount?.count || 0),
      sent: sentTotal,
...
export async function acceptInterestFromInbox(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function declineInterestFromInbox(req, res) {
  try {
    const userId = req.user.user_id;
...
export async function getCancelledInvitations(req, res) {
  try {
    const userId = req.user.user_id;
    const { page = 1, limit = 10 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
...
export async function getMatchesForInbox(req, res) {
  try {
    const userId = req.user.user_id;
    ...
    const queryParams = [
      userId, userId, userId, userId, ...genderParam, userId, userId, userId, userId, parseInt(limit as string), offset
    ];
    ...
    const enrichedProfiles = await Promise.all(filteredProfiles.map(async (profile: any) => {
      const photos = await query(
        `SELECT id, photo_url, is_primary FROM user_photos WHERE user_id = ? ORDER BY is_primary DESC, id ASC`,
        [profile.id]
      );
      const [ps] = await query(
        `SELECT album_photo_privacy FROM privacy_settings WHERE user_id = ?`,
        [profile.id]
      );
      const ap = ps?.album_photo_privacy ?? 'visible_to_all';
      ...
      return { ...profile, photos, album_privacy_status: albumPrivacyStatus, album_count: photos.filter((p: any) => !p.is_primary).length };
    }));
    const [{ total }] = await query(`
