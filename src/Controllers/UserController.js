...
exports.sendNameUpdateOTP = sendNameUpdateOTP;
exports.updateNameWithOTP = updateNameWithOTP;
exports.sendEmailVerificationOTPOld = sendEmailVerificationOTPOld;
exports.sendEmailVerificationOTP = sendEmailVerificationOTP;
exports.verifyEmailOTPOld = verifyEmailOTPOld;
exports.createProfileOld = createProfileOld;
exports.createProfile = createProfile;
exports.getProfile = getProfile;
exports.updateProfile = updateProfile;
exports.updateReligiousInfoOld = updateReligiousInfoOld;
exports.updateReligiousInfo = updateReligiousInfo;
exports.updateGovernmentId = updateGovernmentId;
exports.completeProfile = completeProfile;
exports.getMasterData = getMasterData;
exports.getSubscriptionRestrictionStatus = getSubscriptionRestrictionStatus;
exports.uploadAboutMyselfAudio = uploadAboutMyselfAudio;
...
const MatchActionsController_1 = require("../Controllers/MatchActionsController");
const db = require("../database");
const query = utils.promisify(db.query).bind(db);
const JWT_SECRET = process.env.JWT_SECRET_KEY;
const PROFILE_CREATED_BY = ['self', 'parent', 'sibling', 'relative', 'friend'];
function definedFields(mapping) {
    const out = {};
...
async function upsertByUser(table, userId, fields) {
    ...
    if (existing) {
        await query(`UPDATE ${table} SET ${cols.map((col) => `${col} = ?`).join(", ")} WHERE user_id = ?`, [...cols.map((col) => fields[col]), userId]);
    }
    else {
        await query(`INSERT INTO ${table} (user_id, ${cols.join(", ")}) VALUES (?, ${cols.map(() => "?").join(", ")})`, [userId, ...cols.map((col) => fields[col])]);
    }
}
function normalizeManglik(value) {
    ...
    if (text === "no")
        return "no";
    if (text.includes("anshik") || text.includes("partial"))
        return "yes";
    return "dont_know";
}
...
async function sendMatchingProfilesEmail(userId, email, firstName, religionId, communityId, genderId) {
    ...
    `;
        await EmailService_1.EmailService.sendTemplateEmail('welcome_matches', email, { user_name: firstName, match_count: matchingProfiles.length }, {
...
async function sendNameUpdateOTP(req, res) {
    try {
        ...
        // Get user details
        const [user] = await query("SELECT email, phone FROM users WHERE id = ?", [userId]);
        if (!user) {
        ...
        await query("INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at) VALUES (?, ?, 'name_update', ?, ?) ON DUPLICATE KEY UPDATE otp_code = VALUES(otp_code), expires_at = VALUES(expires_at), created_at = NOW()", [userId, otp, email, expiresAt]);
        // Send OTP via email
        try {
            await EmailService_1.EmailService.sendTemplateEmail('name_update_otp', email, { user_name: email, otp }, {
...
async function updateNameWithOTP(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        const [otpRecord] = await query("SELECT otp_code, expires_at FROM otp_verification WHERE user_id = ? AND otp_type = 'name_update' AND expires_at > NOW() AND is_verified = FALSE ORDER BY created_at DESC LIMIT 1", [userId]);
        if (!otpRecord) {
            return res.status(400).json({
                success: false,
        ...
        // Update name
        await query("UPDATE user_profiles SET first_name = ?, middle_name = ?, last_name = ? WHERE user_id = ?", [first_name, middle_name || null, last_name, userId]);
        // Mark OTP as verified
        await query("UPDATE otp_verification SET is_verified = TRUE, verified_at = NOW() WHERE user_id = ? AND otp_type = 'name_update'", [userId]);
        res.json({
            success: true,
...
async function sendEmailVerificationOTPOld(req, res) {
    try {
        const { email } = req.body;
        if (!email) {
            return res.status(400).json({
        ...
        await query("UPDATE users SET email = CONCAT(email, '_deleted_', id) WHERE email = ? AND status = 4", [email]);
        // Clear any old OTP verification records for this email so re-registration works
        await query("DELETE FROM otp_verification WHERE contact_info = ? AND otp_type = 'email_verification'", [email]);
        // Generate OTP
        const otp = generateOTP();
        const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
        // Store OTP
        await query("INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at) VALUES (0, ?, 'email_verification', ?, ?) ON DUPLICATE KEY UPDATE otp_code = VALUES(otp_code), expires_at = VALUES(expires_at), created_at = NOW()", [otp, email, expiresAt]);
        console.log(`\n========================================`);
        ...
        console.log(`========================================\n`);
        // Send OTP via email
        await EmailService_1.EmailService.sendTemplateEmail('email_verification', email, { user_name: email, otp }, {
...
async function sendEmailVerificationOTP(req, res) {
    try {
        ...
        await query("DELETE FROM otp_verification WHERE contact_info = ? AND otp_type = 'registration' AND is_verified = FALSE", [phone]);
        await query("INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at) VALUES (0, ?, 'registration', ?, ?)", [otp, phone, expiresAt]);
        console.log(`\n========================================`);
...
// Verify Email OTP
async function verifyEmailOTPOld(req, res) {
    try {
        ...
        // Verify OTP
        const [otpRecord] = await query("SELECT otp_code, expires_at FROM otp_verification WHERE contact_info = ? AND otp_type = 'email_verification' AND expires_at > NOW() AND is_verified = FALSE ORDER BY created_at DESC LIMIT 1", [email]);
        ...
        // Mark OTP as verified
        await query("UPDATE otp_verification SET is_verified = TRUE, verified_at = NOW() WHERE contact_info = ? AND otp_type = 'email_verification'", [email]);
        res.json({
            success: true,
...
// Create User Profile
async function createProfileOld(req, res) {
    ...
    try {
        ...
        const { profile_for, first_name, middle_name, last_name, religion_id, community_id, email, date_of_birth, country_id, phone, country_code_id, password, gender_id, caste_id, city, city_id, lives_with_family, family_location, marital_status_id, has_children, number_of_children, height, diet, diet_id, education_level, college_name, college_name_2, annual_income, income_type, work_type, occupation, company_name, about_myself, exclude_from_matchmaking, mother_tongue_id, } = req.body;
        ...
        if (city_id) {
            ...
            if (cityRow) {
                ...
                if (cityRow.state_id) {
                    ...
                    if (stateRow)
                        resolvedStateName = stateRow.state_name;
                }
            }
        }
        ...
        if (!google_verified) {
            const [emailVerified] = await query("SELECT is_verified FROM otp_verification WHERE contact_info = ? AND otp_type = 'email_verification' AND is_verified = TRUE ORDER BY verified_at DESC LIMIT 1", [email]);
            if (!emailVerified) {
                return res.status(400).json({
                    success: false,
        ...
        await query("UPDATE users SET email = CONCAT(email, '_deleted_', id), phone = CASE WHEN phone IS NOT NULL THEN CONCAT(phone, '_deleted_', id) ELSE phone END WHERE email = ? AND status = 4", [email]);
        if (phone) {
            await query("UPDATE users SET phone = CONCAT(phone, '_deleted_', id) WHERE phone = ? AND status = 4", [phone]);
        }
        // Hash password
        ...
        const userResult = await query("INSERT INTO users (email, password, phone, vivaaha_user_id, country_code_id, user_type_id, status, email_verified) VALUES (?, ?, ?, ?, ?, 1, 1, TRUE)", [email, hashedPassword, phone, vivahaUserId, country_code_id || 1]);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (education_level || college_name) {
            ...
            await query("INSERT INTO education_details (user_id, education_level_id, institution_name, institution_name_2) VALUES (?, ?, ?, ?)", [userId, education_level || 1, college_name, college_name_2 || null]);
        ...
        if (occupation || company_name || annual_income) {
            ...
            await query(`INSERT INTO career_details (user_id, occupation, company_name, annual_income, income_type, currency_id, country_living_in_id, city_living_in, working_with_id, working_as, employer_name, highest_qualification, college_attended, state_living_in_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, resolvedOccupation, company_name, annual_income, income_type || 'yearly', 1, resolvedCountryLivingInId, resolvedCityName || null, resolvedWorkingWithId, resolvedOccupation, company_name, resolvedEducationLevelName, college_name || null, resolvedStateId || null]);
        ...
        if (city || city_id) {
            ...
            await query("INSERT INTO address_details (user_id, address_type, city, city_id, country_id) VALUES (?, 'current', ?, ?, ?)", [userId, resolvedCityName, city_id, country_id || 1]);
        ...
        await query("INSERT INTO location_details (user_id, current_residence, city_id, state_id, state_living_in, country_id) VALUES (?, ?, ?, ?, ?, ?)", [userId, resolvedCityName || null, city_id || null, resolvedStateId, resolvedStateName || null, country_id || 1]);
        console.log("✅ location_details inserted");
        // Generate access token
        const token = jwt.sign({ user_id: userId, email: email, user_type: "user" }, JWT_SECRET, { expiresIn: "24h" });
        // Generate and send default password via email
        const defaultPassword = generatePassword();
        const hashedDefaultPassword = await bcrypt.hash(defaultPassword, 10);
        // Update user with default password
        await query("UPDATE users SET password = ? WHERE id = ?", [hashedDefaultPassword, userId]);
        // Send password via email
        try {
            await EmailService_1.EmailService.sendTemplateEmail('user_registration', email, { user_name: `${first_name} ${last_name}`, email, temp_password: defaultPassword, profile_id: vivahaUserId || userId }, {
        ...
        console.log("\n🔍 MISSING DATA CHECK:");
        console.log("   - career.grew_up_in_ids:", ((_a = completeProfile.career) === null || _a === void 0 ? void 0 : _a.grew_up_in_ids) || "❌ MISSING");
        console.log("   - career.ethnic_origin_id:", ((_b = completeProfile.career) === null || _b === void 0 ? void 0 : _b.ethnic_origin_id) || "❌ MISSING");
        console.log("   - career.country_living_in_id:", ((_c = completeProfile.career) === null || _c === void 0 ? void 0 : _c.country_living_in_id) || "❌ MISSING");
        console.log("   - career.city_living_in:", ((_d = completeProfile.career) === null || _d === void 0 ? void 0 : _d.city_living_in) || "❌ MISSING");
        console.log("   - location.state_id:", ((_e = completeProfile.location) === null || _e === void 0 ? void 0 : _e.state_id) || "❌ MISSING");
        console.log("   - location.city_name:", ((_f = completeProfile.location) === null || _f === void 0 ? void 0 : _f.city_name) || "❌ MISSING");
        console.log("   - basic.community_id:", ((_g = completeProfile.basic) === null || _g === void 0 ? void 0 : _g.community_id) || "❌ MISSING");
        console.log("   - basic.mother_tongue_id:", ((_h = completeProfile.basic) === null || _h === void 0 ? void 0 : _h.mother_tongue_id) || "❌ MISSING");
...
async function createProfile(req, res) {
    ...
    try {
        ...
        if (existingPhone) {
            ...
            const existingToken = jwt.sign({ user_id: existingPhone.id, user_type: "user" }, JWT_SECRET, { expiresIn: "24h" });
            return res.status(200).json({
                success: true,
                message: "User Information",
                is_existing_user: true,
                user_id: existingPhone.id,
                vivaaha_user_id: ((_a = profile.basic) === null || _a === void 0 ? void 0 : _a.vivaaha_user_id) || null,
                access_token: existingToken,
                profile_for: ((_b = profile.basic) === null || _b === void 0 ? void 0 : _b.profile_created_by) || null,
                phone,
                profile
            });
        }
        const [verifiedOtp] = await query("SELECT id FROM otp_verification WHERE contact_info = ? AND otp_type = 'phone' AND is_verified = TRUE ORDER BY verified_at DESC, created_at DESC LIMIT 1", [phone]);
        if (!verifiedOtp) {
            return res.status(400).json({
                success: false,
        ...
        const vivahaUserId = await (0, MatchActionsController_1.generateUniqueVivahaId)();
        const userResult = await query("INSERT INTO users (phone, country_code_id, vivaaha_user_id, user_type_id, status, phone_verified) VALUES (?, ?, ?, 1, 1, TRUE)", [phone, country_code_id || 1, vivahaUserId]);
        const userId = userResult.insertId;
        await query("INSERT INTO user_profiles (user_id, profile_created_by, aadhaar_verified) VALUES (?, ?, 1)", [userId, profileCreatedBy]);
        const token = jwt.sign({ user_id: userId, user_type: "user" }, JWT_SECRET, { expiresIn: "24h" });
        res.status(201).json({
            success: true,
            message: "Profile created successfully",
            user_id: userId,
...
async function getProfile(req, res) {
    try {
        ...
        const basicResult = await query(`SELECT u.vivaaha_user_id, up.age, up.date_of_birth, up.height, up.marital_status_id, up.blood_group_id,
       ...
       WHERE up.user_id = ?`, [userId]);
        ...
        const astroResult = await query(`SELECT ad.*, g.gothra_name, c.country_name
       FROM astro_details ad
       LEFT JOIN gothra_master g ON ad.gothra_id = g.id
       LEFT JOIN country_code_master c ON ad.country_of_birth_id = c.id
       WHERE ad.user_id = ?`, [userId]);
        const familyResult = await query(`SELECT fd.*, po1.occupation_name as father_occupation_name, po2.occupation_name as mother_occupation_name,
              ffs.status_name as financial_status, c.country_name as family_country
       FROM family_details fd
       LEFT JOIN parent_occupation_master po1 ON fd.father_occupation_id = po1.id
       LEFT JOIN parent_occupation_master po2 ON fd.mother_occupation_id = po2.id
       ...
       LEFT JOIN country_code_master c ON fd.family_country_id = c.id
       WHERE fd.user_id = ?`, [userId]);
        const careerResult = await query(`SELECT cd.*, ww.working_type, c.country_name as country_living,
              CASE WHEN cd.occupation REGEXP '^[0-9]+$' THEN COALESCE(pm.profession_name, cd.occupation) ELSE cd.occupation END as occupation
       FROM career_details cd
       LEFT JOIN working_with_master ww ON cd.working_with_id = ww.id
       LEFT JOIN country_code_master c ON cd.country_living_in_id = c.id
       ...
       WHERE cd.user_id = ?`, [userId]);
        ...
        const locationResult = await query(`SELECT ld.*, c.city_name, s.state_name, co.country_name FROM location_details ld
       LEFT JOIN cities_master c ON ld.city_id = c.id
       LEFT JOIN states_master s ON ld.state_id = s.id
       LEFT JOIN country_code_master co ON ld.country_id = co.id
       WHERE ld.user_id = ?`, [userId]);
       ...
       JOIN hobbies_master hm ON uh.hobby_id = hm.id
       WHERE uh.user_id = ?`, [userId]);
        const governmentIdResult = await query(`SELECT ugiv.id_type_id, ugiv.id_number, ugiv.is_verified, gitm.id_type_name
       FROM user_government_id_verification ugiv
       LEFT JOIN government_id_type_master gitm ON ugiv.id_type_id = gitm.id
       WHERE ugiv.user_id = ?`, [userId]);
        const [subscription] = await query(`SELECT us.*, sp.plan_name, sp.price, sp.duration_months,
       ...
       ORDER BY us.created_at DESC LIMIT 1`, [userId]);
        ...
        console.log("   hobbies count:", (hobbies === null || hobbies === void 0 ? void 0 : hobbies.length) || 0);
...
async function updateProfile(req, res) {
    try {
        const userId = req.user.user_id;
        const { first_name, middle_name, last_name, height, weight, about_me, mother_tongue, occupation, company_name, annual_income, } = req.body;
        // Convert mother_tongue to integer or null
        const motherTongueId = mother_tongue ? parseInt(mother_tongue) || null : null;
        // Update profile
        await query(`UPDATE user_profiles SET first_name = ?, middle_name = ?, last_name = ?, height = ?,
       weight = ?, about_me = ?, mother_tongue_id = ? WHERE user_id = ?`, [first_name, middle_name || null, last_name, height, weight, about_me, motherTongueId, userId]);
        // Update career if provided
        if (occupation || company_name || annual_income) {
            const [existingCareer] = await query("SELECT id FROM career_details WHERE user_id = ?", [userId]);
            if (existingCareer) {
                await query("UPDATE career_details SET occupation = ?, company_name = ?, annual_income = ? WHERE user_id = ?", [occupation, company_name, annual_income, userId]);
            }
            else {
                await query("INSERT INTO career_details (user_id, occupation, company_name, annual_income, currency_id) VALUES (?, ?, ?, ?, 1)", [userId, occupation, company_name, annual_income]);
            }
        }
        res.json({
            success: true,
...
async function updateReligiousInfoOld(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        await query(`UPDATE user_profiles SET religion_id = ?, mother_tongue_id = ?, community_id = ?, caste_id = ?
       WHERE user_id = ?`, [religion_id, mother_tongue_id || null, community_id || null, caste_id || null, userId]);
        // Update or create astro details for gothra
        if (gothra_id) {
            const [existingAstro] = await query("SELECT id FROM astro_details WHERE user_id = ?", [userId]);
            if (existingAstro) {
                await query("UPDATE astro_details SET gothra_id = ? WHERE user_id = ?", [gothra_id, userId]);
            }
            else {
                await query("INSERT INTO astro_details (user_id, gothra_id) VALUES (?, ?)", [userId, gothra_id]);
            }
        }
        res.json({
            success: true,
...
// Update Religious Information
async function updateReligiousInfo(req, res) {
    try {
        const userId = req.user.user_id;
        const { religion_id, mother_tongue_id, community_id, caste_id, gothra_id } = req.body;
...
async function updateGovernmentId(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        const [existing] = await query("SELECT id FROM user_government_id_verification WHERE user_id = ?", [userId]);
        if (existing) {
            // Update existing record
            await query("UPDATE user_government_id_verification SET id_type_id = ?, id_number = ?, is_verified = FALSE, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?", [id_type_id, id_number, userId]);
        }
        else {
            // Create new record
...
async function markUserFullyVerified(userId) {
    ...
    await query("UPDATE users SET phone_verified = 1 WHERE id = ?", [userId]);
    for (const docType of ["aadhaar", "pan", "driving_license"]) {
        const [existing] = await query("SELECT id FROM document_verification WHERE user_id = ? AND document_type = ? LIMIT 1", [userId, docType]);
        if (existing) {
            await query("UPDATE document_verification SET verification_status = 'verified', verified_at = NOW(), updated_at = NOW() WHERE id = ?", [existing.id]);
        }
        else {
            await query("INSERT INTO document_verification (user_id, document_type, document_number, verification_status, verified_at) VALUES (?, ?, '', 'verified', NOW())", [userId, docType]);
        }
    }
}
...
async function getSubscriptionRestrictionStatus(req, res) {
    try {
        const [general] = await query(`SELECT subscription_restrictions
       FROM general_settings
       LIMIT 1`);
        res.json({
            success: true,
            subscription_restrictions: Number((general === null || general === void 0 ? void 0 : general.subscription_restrictions) || 0)
        });
    }
    catch (error) {
...
async function uploadAboutMyselfAudio(req, res) {
    try {
        const userId = req.user.user_id;
        ...
        audioUpload(req, res, async (err) => {
            ...
            try {
                ...
                // Save to database
                await query("INSERT INTO user_audio_files (user_id, s3_path, file_size) VALUES (?, ?, ?)", [userId, audioUrl, req.file.size]);
                res.json({
                    success: true,
...
async function verifyEmailOTP(req, res) {
    try {
        ...
        const [otpRecord] = await query("SELECT id, otp_code FROM otp_verification WHERE contact_info = ? AND otp_type = 'registration' AND expires_at > NOW() AND is_verified = FALSE ORDER BY created_at DESC LIMIT 1", [phone]);
        if (!otpRecord || otpRecord.otp_code !== otp) {
            return res.status(400).json({
                success: false,
                message: "Invalid or expired OTP"
        ...
        await query("UPDATE otp_verification SET is_verified = TRUE, verified_at = NOW() WHERE id = ?", [otpRecord.id]);
        res.json({
            success: true,
