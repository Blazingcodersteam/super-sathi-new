...
async function resolveRecipient(body: any) {
  ...
  if (phone) {
    const [user] = await query(
      `SELECT id, phone FROM users WHERE phone = ? AND status != 4 LIMIT 1`,
      [phone]
    );
    return { user, phone };
  }
  ...
  if (email_id) {
    const [user] = await query(
      `SELECT id, phone FROM users WHERE email = ? AND status != 4 LIMIT 1`,
      [email_id]
    );
...
export async function sendOtp(req, res) {
  try {
    ...
    const otpInsertResult: any = await query(
      `INSERT INTO otp_verification (user_id, otp_code, otp_type, contact_info, expires_at)
       VALUES (?, ?, 'phone', ?, ?)`,
      [0, otp, phone, expiresAt]
    );
    ...
    console.log("[sendOtp] OTP insert result", {
      ...
      affectedRows: otpInsertResult?.affectedRows,
...
export async function verifyOtp(req, res) {
  try {
    ...
    await query(
      `UPDATE otp_verification SET is_verified = 1, verified_at = NOW() WHERE id = ?`,
      [otpRow.id]
    );
